...
Deno.serve(async (req) => {
  ...
  try {
    const { house_name, notes, submitted_by } = await req.json();
    ...
    const { data: settings } = await supabase
      ...
      .select('user_id')
      .eq('notification_type', 'supplies_inventory');
    ...
    const html = wrap(`
      <h2 style="margin:0 0 20px;font-size:20px;color:#1a1a1a;">📦 Supplies/Inventory Entry</h2>
      <p style="margin:0 0 20px;color:#555;">A new Supplies/Inventory entry has been logged for <strong>${house_name}</strong>.</p>
      <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border:1px solid #eee;border-radius:8px;overflow:hidden;">
        <tr><td style="padding:10px 12px;background:#f9f9f9;border-bottom:1px solid #eee;font-size:13px;color:#666;width:40%;">House</td><td style="padding:10px 12px;background:#f9f9f9;border-bottom:1px solid #eee;font-size:14px;color:#333;">${house_name}</td></tr>
        <tr><td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px;color:#666;">Submitted By</td><td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:14px;color:#333;">${submitted_by}</td></tr>
        <tr><td style="padding:10px 12px;background:#f9f9f9;font-size:13px;color:#666;">Notes</td><td style="padding:10px 12px;background:#f9f9f9;font-size:14px;color:#333;">${notes || 'No notes provided.'}</td></tr>
      </table>
    `);
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to: emails,
        subject: `Supplies/Inventory Entry — ${house_name}`,
        html,
      }),
    });
    ...
    console.error('supplies-inventory-notify error:', err);
