...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const html = wrap(`
      ...
      <p style="margin:0 0 20px;color:#555;"><strong>${client_name}</strong> has been confirmed as a new resident at <strong>${house_name}</strong>.</p>
      <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border:1px solid #eee;border-radius:8px;overflow:hidden;">
        ...
        <tr><td style="padding:10px 12px;background:#f9f9f9;border-bottom:1px solid #eee;font-size:13px;color:#666;">Move-In Date</td><td style="padding:10px 12px;background:#f9f9f9;border-bottom:1px solid #eee;font-size:14px;color:#333;">${move_in_date}</td></tr>
        <tr><td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px;color:#666;">Starting Level</td><td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:14px;color:#333;">Level ${level}</td></tr>
        <tr><td style="padding:10px 12px;background:#f9f9f9;font-size:13px;color:#666;">Early Admission</td><td style="padding:10px 12px;background:#f9f9f9;font-size:14px;font-weight:600;color:${early_admission ? '#b45309' : '#333'};">${early_admission ? '⭐ Yes' : 'No'}</td></tr>
      </table>
    `);
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to: emails,
        subject: `Move-In Confirmed — ${client_name} · ${house_name}`,
        html,
      }),
    });
    ...
    console.error('confirmed-move-in-notify error:', err);
