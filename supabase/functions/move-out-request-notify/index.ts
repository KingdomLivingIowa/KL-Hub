...
Deno.serve(async (req) => {
  ...
  try {
    const { client_id, house_id, client_name, move_out_date } = await req.json();
    ...
    const emailHtml = wrap(`
      ...
      <p style="margin:0 0 16px;color:#555;"><strong>${client_name}</strong> from <strong>${houseName}</strong> has submitted a move-out request.</p>
      <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:20px;">
        ...
        <tr><td style="padding:10px 12px;background:#f9f9f9;font-size:13px;color:#666;">Requested Move-Out Date</td><td style="padding:10px 12px;background:#f9f9f9;font-size:14px;color:#b22222;font-weight:600;">${formattedDate}</td></tr>
      </table>
      <p style="color:#888;font-size:13px;">Please log into KL Hub to review and approve or deny this request.</p>
    `);
