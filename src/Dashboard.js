...
function DashboardHome({ counts, currentUser }) {
  ...
  return (
    <div>
      ...
      {loadingDashboard ? (
        <p style={{ color: '#52525b', fontSize: '14px', marginTop: '32px' }}>Loading dashboard...</p>
      ) : (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <Section title="Waiting Lists" count={totalWaiting} countColor="#c2410c">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <p style={{ fontSize: '10px', color: '#52525b', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 4px 0', fontWeight: '600' }}>Men's</p>
                {['DOC Men', 'Community Men', 'Treatment Men'].map(list => (
                  <div key={list} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: '#ffffff', borderRadius: '8px', border: '1px solid #d4d4da' }}>
                    <span style={{ fontSize: '13px', color: '#52525b' }}>{list}</span>
                    <span style={{ fontSize: '15px', fontWeight: '700', color: waitingListCounts[list] > 0 ? '#2563eb' : '#4b5563' }}>{waitingListCounts[list] || 0}</span>
                  </div>
                ))}
                <p style={{ fontSize: '10px', color: '#52525b', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '10px 0 4px 0', fontWeight: '600' }}>Women's</p>
                {['DOC Women', 'Community Women', 'Treatment Women'].map(list => (
                  <div key={list} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: '#ffffff', borderRadius: '8px', border: '1px solid #d4d4da' }}>
                    <span style={{ fontSize: '13px', color: '#52525b' }}>{list}</span>
                    <span style={{ fontSize: '15px', fontWeight: '700', color: waitingListCounts[list] > 0 ? '#db2777' : '#4b5563' }}>{waitingListCounts[list] || 0}</span>
                  </div>
                ))}
              </div>
            </Section>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  {readNotifications.length > 0 && (
                    <div style={{ marginTop: '4px' }}>
                      <button onClick={() => setShowReadNotifications(p => !p)}
                        style={{ background: 'transparent', border: 'none', color: '#71717a', fontSize: '11px', padding: '4px 0', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      ...
                      {showReadNotifications && readNotifications.map(notif => (
                        <div key={notif.id} style={{ background: '#f7f7f9', border: '1px solid #c9c9cf', borderRadius: '10px', padding: '10px 14px', display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '6px', opacity: 0.5 }}>
                          <span style={{ fontSize: '13px', flexShrink: 0 }}>{notifIcon(notif.type)}</span>
                          <p style={{ color: '#4b5563', fontSize: '12px', margin: 0, flex: 1 }}>{notif.message}</p>
                          <span style={{ color: '#4b5563', fontSize: '11px', flexShrink: 0 }}>{formatTimeAgo(notif.created_at)}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {unreadNotifications.length > 1 && (
                    <button onClick={markAllNotificationsRead}
                      style={{ alignSelf: 'flex-end', background: 'transparent', border: '1px solid #b0b0b7', color: '#52525b', fontSize: '11px', padding: '4px 10px', borderRadius: '6px', cursor: 'pointer', marginTop: '4px' }}>
                      Mark all read
                    </button>
                  )}
                  {/* Empty state for house managers */}
                  {alerts.length === 0 && notifications.length === 0 && (
                    <p style={{ color: '#71717a', fontSize: '13px', textAlign: 'center', padding: '12px 0', margin: 0 }}>No new notifications</p>
                  )}
                </div>
              </Section>
            )}
            ...
            <Section title="Bed Availability">
              {houses.length === 0 ? (
                <p style={{ color: '#52525b', fontSize: '14px' }}>No houses found.</p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {houses.map(h => {
                    ...
                    return (
                      <div key={h.id} style={{ background: '#f7f7f9', borderRadius: '10px', padding: '12px 14px', border: '1px solid #c9c9cf' }}>
                        ...
                        <div style={{ display: 'flex', gap: '14px' }}>
                          <BedStat label="Total" value={h.total_beds || 0} color="#52525b" />
              ...
              <Section title="Open Charges">
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: '#fee2e2', borderRadius: '10px', border: '1px solid #fecaca', marginBottom: '14px' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <p style={{ color: '#52525b', fontSize: '11px', margin: '2px 0 0 0' }}>client{new Set(openCharges.map(c => c.client_id)).size !== 1 ? 's' : ''} with balance</p>
                  </div>
                </div>
                      ...
                      <div key={houseId}>
                        {!isHouseManagerRole && (
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                            <p style={{ fontSize: '11px', color: '#4b5563', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0, fontWeight: '600' }}>{group.name}</p>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          {Object.values(clientMap).map((client, i) => (
                            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 10px', background: '#ffffff', borderRadius: '7px', border: '1px solid #c9c9cf' }}>
...
function Section({ title, children, count, countColor }) {
  ...
  return (
    <div style={{ background: '#f7f7f9', borderRadius: '12px', border: '1px solid #c9c9cf', overflow: 'hidden' }}>
      <div onClick={toggle} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', cursor: 'pointer', userSelect: 'none' }}>
        ...
        <span style={{ color: '#71717a', fontSize: '13px', display: 'inline-block', transform: collapsed ? 'rotate(-90deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}>▾</span>
      </div>
      {!collapsed && (
...
function BedStat({ label, value, color }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>
      ...
      <span style={{ fontSize: '10px', color: '#52525b', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{label}</span>
    </div>
  );
}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              ...
              <p style={{ color: '#71717a', fontSize: '11px', margin: 0 }}>Parole Officer Portal</p>
            </div>
          </div>
          <button onClick={() => supabase.auth.signOut()} style={{ background: 'transparent', border: '1px solid #c9c9cf', color: '#6b7280', padding: '6px 14px', borderRadius: '8px', fontSize: '13px', cursor: 'pointer' }}>
            Sign Out
          </button>
        </div>
        <PODashboard />
