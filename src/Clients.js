...
function generateStayHistoryPDF(stay, client, history, logoSrc) {
  ...
  const checkinRows = history.checkIns.length === 0
    ? '<p style="color:#6b7280;font-size:13px;padding:6px 0;">No weekly check-ins during this stay.</p>'
  ...
  const formsRows = (history.overnights.length === 0 && !history.welcomePacket)
    ? '<p style="color:#6b7280;font-size:13px;padding:6px 0;">No forms submitted during this stay.</p>'
    : `${history.welcomePacket ? `<div style="padding:8px 0;border-bottom:1px solid #c9c9cf;"><strong style="font-size:13px;">Welcome Packet</strong> <span style="font-size:12px;color:#6b7280;">— Completed ${fmtFull(history.welcomePacket.created_at)}</span></div>` : ''}
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .report-sub { font-size: 13px; color: #6b7280; margin-bottom: 8px; }
    .print-btn { position: fixed; top: 16px; right: 16px; padding: 10px 20px; background: #8b1c1c; color: white; border: none; border-radius: 6px; font-size: 14px; cursor: pointer; }
...
function generateProgressReportPDF(client, uaRecords, meetingRecords, choreRecords, stays, checkIn, logoSrc) {
  ...
  ${!checkIn ? '<p style="color:#6b7280;font-size:14px;padding:8px 0;">No weekly check-ins on record.</p>' : `
  <div class="checkin-box">
    ...
    ${checkIn.notes ? `<div style="margin-top:10px;padding-top:10px;border-top:1px solid #3f3f46;"><div style="font-size:12px;color:#6b7280;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:4px;">Weekly reflection</div><div style="font-size:14px;color:#c9c9cf;line-height:1.6;">${checkIn.notes}</div></div>` : ''}
  </div>`}
...
function generateUAHistoryPDF(client, uaRecords, logoSrc) {
  ...
  const resultColor = (r) => r === 'Negative' ? '#16a34a' : r === 'Positive' ? '#dc2626' : r === 'Inconclusive' ? '#b45309' : '#6b7280';
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sum-label { font-size: 12px; color: #6b7280; margin-top: 3px; }
    table { width: 100%; border-collapse: collapse; font-size: 14px; }
    th { background: #f4f4f6; color: #18181b; padding: 10px 12px; text-align: left; font-size: 13px; }
    td { padding: 10px 12px; border-bottom: 1px solid #c9c9cf; }
  ...
  <div class="summary">
    ...
    <div class="sum-box"><div class="sum-num" style="color:#6b7280;">${incCount + refCount}</div><div class="sum-label">Inc. / Refused</div></div>
  </div>

  ${uaRecords.length === 0 ? '<p style="color:#6b7280;font-size:14px;">No UA records on file.</p>' : `
  <table>
...
function InvitePortalButton({ client }) {
  ...
  const btnStyles = {
    idle:    { background: 'transparent', border: '1px solid #b8b8bf', color: '#52525b' },
    sending: { background: 'transparent', border: '1px solid #b8b8bf', color: '#52525b', opacity: 0.6 },
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={e => { e.stopPropagation(); setOpen(o => !o); }}
        style={{ background: '#f7f7f9', border: '1px solid #b8b8bf', color: '#3f3f46', fontSize: '14px', padding: '5px 10px', borderRadius: '7px', cursor: 'pointer', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '5px', whiteSpace: 'nowrap' }}>
        Move to {open ? '▲' : '▼'}
      </button>
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 6px)', right: 0, background: '#ffffff', border: '1px solid #c9c9cf', borderRadius: '10px', overflow: 'hidden', zIndex: 100, minWidth: '160px', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
          {statuses.map(ns => (
            <button key={ns} onClick={e => { e.stopPropagation(); setOpen(false); onSelect(ns); }}
              style={{ display: 'block', width: '100%', textAlign: 'left', padding: '10px 14px', background: 'transparent', border: 'none', color: '#3f3f46', fontSize: '14px', cursor: 'pointer', borderBottom: '1px solid #c9c9cf' }}
...
function MoveHouseModal({ client, houses, onClose, onSuccess }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }} onClick={onClose}>
      <div style={{ background: '#ffffff', border: '1px solid #c9c9cf', borderRadius: '14px', padding: '28px', width: '100%', maxWidth: '420px', boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginBottom: '14px' }}>
          <label style={{ display: 'block', fontSize: '14px', color: '#52525b', marginBottom: '5px' }}>Destination House *</label>
          <select value={toHouseId} onChange={e => setToHouseId(e.target.value)}
            style={{ width: '100%', background: '#ffffff', border: '1px solid #b8b8bf', borderRadius: '8px', padding: '10px 12px', color: toHouseId ? '#18181b' : '#71717a', fontSize: '14px', boxSizing: 'border-box' }}>
            <option value="">Select a house...</option>
            {eligibleHouses.map(h => <option key={h.id} value={h.id}>{h.name}</option>)}
          </select>
        </div>
        <div style={{ marginBottom: '14px' }}>
          <label style={{ display: 'block', fontSize: '14px', color: '#52525b', marginBottom: '5px' }}>Transfer Date</label>
          <input type="date" value={moveDate} onChange={e => setMoveDate(e.target.value)}
            style={{ width: '100%', background: '#ffffff', border: '1px solid #b8b8bf', borderRadius: '8px', padding: '10px 12px', color: '#18181b', fontSize: '14px', boxSizing: 'border-box' }} />
        </div>
        <div style={{ marginBottom: '20px' }}>
          <label style={{ display: 'block', fontSize: '14px', color: '#52525b', marginBottom: '5px' }}>Note (optional)</label>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <button onClick={onClose} style={{ flex: 1, padding: '11px', background: 'transparent', border: '1px solid #b8b8bf', borderRadius: '9px', color: '#52525b', fontSize: '14px', cursor: 'pointer' }}>
            Cancel
          </button>
          <button onClick={handleMove} disabled={saving || !toHouseId}
            style={{ flex: 1, padding: '11px', background: saving || !toHouseId ? '#c9c9cf' : '#dbeafe', border: '1px solid ' + (saving || !toHouseId ? '#b0b0b7' : '#2563eb'), borderRadius: '9px', color: saving || !toHouseId ? '#71717a' : '#2563eb', fontSize: '14px', fontWeight: '600', cursor: saving || !toHouseId ? 'not-allowed' : 'pointer' }}>
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 16px' }}>
        {fields.filter(([,v]) => v != null && v !== '').map(([label, val]) => (
          <div key={label} style={{ padding: '7px 0', borderBottom: '1px solid #c9c9cf' }}>
            <div style={{ fontSize: 12, color: '#4b5563', marginBottom: 2 }}>{label}</div>
        ...
        {parsedMeds.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            ...
            {parsedMeds.map((med, i) => (
              <div key={i} style={{ background: '#ffffff', borderRadius: 8, padding: '10px 14px', marginBottom: 8, border: '1px solid #c9c9cf' }}>
                ...
                {[['Dosage', med.dosage], ['Times/day', med.intake], ['Notes', med.notes]].filter(([,v]) => v).map(([l,v]) => (
                  <div key={l} style={{ fontSize: 13, color: '#52525b' }}>{l}: {v}</div>
                ))}
              </div>
            ))}
          </div>
...
function WeeklyCheckInCard({ entry }) {
  ...
  return (
    <div style={{ marginTop: '8px' }}>
      ...
      {entry.notes && (
        <div style={{ marginTop: '8px' }}>
          <p style={{ fontSize: '13px', color: '#4b5563', textTransform: 'uppercase', letterSpacing: '0.5px', margin: '0 0 4px 0' }}>Weekly reflection</p>
          <p style={{ fontSize: '14px', color: '#52525b', margin: 0, lineHeight: 1.5 }}>{entry.notes}</p>
        </div>
      )}
    </div>
  );
...
function LatestCheckIn({ clientId }) {
  ...
  const fmtDate = (d) => new Date(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  const Row = ({ label, value }) => value != null && value !== '' ? (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid #c9c9cf' }}>
    ...
    <div>
      ...
      {entry.notes && (
        <div style={{ marginTop: 12 }}>
          <p style={{ fontSize: 12, color: '#4b5563', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 6px 0' }}>Weekly reflection</p>
...
function ClientFormsTab({ client }) {
  ...
  const generateWelcomePDF = (p) => {
    ...
    const row = (label, value) => `<tr><td style="padding:8px 12px;color:#71717a;font-size:13px;width:55%;border-bottom:1px solid #f0f0f0;vertical-align:top;">${label}</td><td style="padding:8px 12px;font-size:13px;border-bottom:1px solid #f0f0f0;vertical-align:top;font-weight:500;">${value || '—'}</td></tr>`;
    const section = (title) => `<tr><td colspan="2" style="padding:14px 12px 6px;background:#f9f9f9;font-size:12px;font-weight:700;color:#b22222;text-transform:uppercase;letter-spacing:1px;border-bottom:1px solid #c9c9cf;">${title}</td></tr>`;
  ...
  if (loading) return <div style={{ padding: '20px', color: '#6b7280' }}>Loading...</div>;

  const s = { padding: '0 2px' };
  const labelStyle = { fontSize: '12px', color: '#71717a', textTransform: 'uppercase', letterSpacing: '0.5px', margin: '0 0 3px' };
  const valueStyle = { fontSize: '14px', color: '#3f3f46', margin: 0 };

  if (!packet) return (
    <div style={{ padding: '40px 20px', textAlign: 'center' }}>
      <p style={{ color: '#6b7280', fontSize: '15px' }}>No welcome packet submitted yet.</p>
      <p style={{ color: '#71717a', fontSize: '14px' }}>The client can submit this from their portal under the Forms tab.</p>
    </div>
  );

  const Row = ({ label, value }) => (
    <div style={{ padding: '8px 0', borderBottom: '1px solid #cfcfd4' }}>
  ...
  return (
    <div style={s}>
      <div onClick={() => setShowPacketDetails(prev => !prev)}
        style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', background: '#ffffff', borderRadius: '10px', border: '1px solid #c9c9cf', cursor: 'pointer', marginBottom: showPacketDetails ? '16px' : '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span style={{ fontSize: '22px' }}>📋</span>
          <div>
            <p style={{ color: '#18181b', fontWeight: '600', margin: '0 0 2px' }}>Welcome Packet</p>
            <p style={{ color: '#6b7280', fontSize: '13px', margin: 0 }}>Program agreements & acknowledgements</p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <span style={{ color: '#4b5563', fontSize: '14px' }}>{showPacketDetails ? '▲' : '▼'}</span>
        </div>
      </div>
          ...
          {overnights.map(req => {
            ...
            return (
              <div key={req.id} style={{ padding: '10px 0', borderBottom: '1px solid #ffffff' }}>
                ...
                <p style={{ color: '#6b7280', fontSize: '13px', margin: '0 0 2px' }}>📍 {req.location || '—'} · 👤 {req.who_seeing || '—'}</p>
                <p style={{ color: '#6b7280', fontSize: '13px', margin: 0 }}>Reason: {req.reason || '—'}</p>
                {req.review_notes && <p style={{ color: '#52525b', fontSize: '13px', margin: '4px 0 0', fontStyle: 'italic' }}>Note: {req.review_notes}</p>}
              </div>
            );
          })}
        </div>
  ...
  return (
    <Card title="Medications" full action={
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {meds.map((med, i) => (
          <div key={i} style={{ background: '#ffffff', borderRadius: '8px', padding: '12px 14px', border: '1px solid #c9c9cf', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
            <div style={{ flex: 1 }}>
              <p style={{ color: '#18181b', fontSize: '14px', fontWeight: '600', margin: '0 0 6px 0' }}>{med.name}</p>
              <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                {med.dosage && <span style={{ fontSize: '13px', color: '#52525b' }}>Dosage: {med.dosage}</span>}
                {med.intake && <span style={{ fontSize: '13px', color: '#52525b' }}>Frequency: {med.intake}x/day</span>}
              </div>
              {med.notes && <p style={{ fontSize: '13px', color: '#4b5563', margin: '6px 0 0 0' }}>{med.notes}</p>}
            </div>
            <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
              <button onClick={() => openEditModal(i)} style={{ background: 'transparent', border: '1px solid #b8b8bf', color: '#52525b', padding: '4px 10px', borderRadius: '6px', fontSize: '13px', cursor: 'pointer' }}>Edit</button>
      ...
      {modalState && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000 }}
          onClick={closeModal}>
          <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #c9c9cf', width: '100%', maxWidth: '420px', padding: '24px' }}
            ...
            {[
              ...
              <div key={field} style={{ marginBottom: '14px' }}>
                <label style={{ fontSize: '12px', color: '#4b5563', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block', marginBottom: '5px' }}>{label}</label>
...
function StayPhotos({ clientId, stayId }) {
  ...
  return (
    <div style={{ gridColumn: 'span 2', marginTop: '8px' }}>
      <p style={{ fontSize: '14px', color: '#52525b', margin: '0 0 8px 0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Discharge Photos</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
        {photos.map((url, i) => (
          <img key={i} src={url} alt="" onClick={() => window.open(url, '_blank')}
            style={{ width: '100px', height: '80px', objectFit: 'cover', borderRadius: '6px', border: '1px solid #c9c9cf', cursor: 'pointer' }} />
        ))}
      </div>
    </div>
  );
...
function Clients({ pendingClientId, onClientOpened, onBackToHouses }) {
  ...
  const statusColor = (s) => {
    ...
    if (s === 'Denied') return { bg: '#c9c9cf', color: '#52525b' };
  ...
  const uaResultColor = (result) => {
    ...
    if (result === 'Refused') return { bg: '#c9c9cf', color: '#52525b' };
    return { bg: '#c9c9cf', color: '#52525b' };
  };
  ...
  const generateTimelinePDF = async (client, startDate, endDate, eventType) => {
    ...
    const rows = entries.map(e => {
      const color = typeColors[e.entry_type] || '#6b7280';
      let detail = '';
  ...
  const EarlyAdmissionField = () => {
    ...
    return (
      <div style={{ padding: '6px 0', borderBottom: '1px solid #c9c9cf' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
          <span style={{ fontSize: '14px', color: '#4b5563', flexShrink: 0 }}>⭐ Early Admission</span>
          {!editing ? (
            <span onClick={() => setEditing(true)} title="Click to edit"
              style={{ fontSize: '14px', color: selected.early_admission ? '#ca8a04' : '#4b5563', cursor: 'text', padding: '1px 4px', borderRadius: '4px', border: '1px solid transparent', transition: 'border-color 0.15s' }}
        ...
        {editing && (
          <div style={{ marginTop: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '6px' }}>
              ...
              <button onClick={cancel} style={{ padding: '4px 12px', background: '#f7f7f9', color: '#52525b', border: '1px solid #c9c9cf', borderRadius: '6px', fontSize: '13px', cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        )}
        {!editing && selected.early_admission && selected.early_admission_notes && (
          <p style={{ fontSize: '13px', color: '#6b7280', margin: '4px 0 0 0', fontStyle: 'italic' }}>{selected.early_admission_notes}</p>
        )}
      </div>
    );
  };
  ...
  const ReadField = ({ label, value, alert: isAlert }) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '3px 0', borderBottom: '1px solid #c9c9cf', gap: '12px' }}>
      <span style={{ fontSize: '14px', color: '#4b5563', flexShrink: 0 }}>{label}</span>
      <span style={{ fontSize: '14px', color: isAlert ? '#dc2626' : value ? '#3f3f46' : '#4b5563', textAlign: 'right', wordBreak: 'break-word' }}>{value || '—'}</span>
    </div>
  );
  ...
  const MeetingWeek = ({ weekStart, entries }) => {
    ...
    return (
      <div style={{ background: '#ffffff', borderRadius: '10px', border: `1px solid ${isThisWeek ? '#dbeafe' : '#c9c9cf'}`, marginBottom: '10px', overflow: 'hidden' }}>
        <div onClick={() => toggleWeek(key)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', cursor: 'pointer' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ color: '#52525b', fontSize: '14px' }}>{isExpanded ? '▲' : '▼'}</span>
          </div>
        </div>
        {isExpanded && (
          <div style={{ borderTop: '1px solid #c9c9cf', padding: '10px 16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {entries.map(m => (
  ...
  const ChorePeriodCard = ({ record }) => {
    ...
    return (
      <div style={{ background: '#ffffff', borderRadius: '10px', border: `1px solid ${isCurrent ? '#dcfce7' : '#c9c9cf'}`, marginBottom: '10px', padding: '12px 16px' }}>
        ...
        <p style={{ fontSize: '14px', color: '#4b5563', margin: '0 0 8px' }}>
            ...
            {allDays.map(d => {
              ...
              return (
                <span key={d} title={d} style={{
                  ...
                  background: done ? '#dcfce7' : isFuture ? '#c9c9cf' : '#fee2e2',
                  color: done ? '#16a34a' : isFuture ? '#71717a' : '#dc2626',
                }}>
                  {done ? '✓' : isFuture ? '·' : '✗'}
                </span>
              );
  ...
  return (
    <div style={st.page}>
      ...
      {loading ? (
        <p style={{ color: '#52525b', padding: '20px' }}>Loading clients...</p>
      ) : clients.length === 0 ? (
        <p style={{ color: '#52525b', padding: '20px' }}>{isHouseManagerRole ? 'No clients found in your assigned house(s).' : 'No clients found.'}</p>
      ) : (
        <>
          <div style={st.table}>
            ...
            {clients.map(c => (
              <div key={c.id} style={st.row} onClick={() => openProfile(c)}>
                ...
                <span style={{ flex: 1, color: '#52525b' }}>{c.status === 'Active' && c.level ? `Level ${c.level}` : '—'}</span>
                <span style={{ flex: 2, color: '#52525b' }}>{c.house_name || '—'}</span>
                <span style={{ flex: 1, color: '#52525b' }}>{c.start_date || '—'}</span>
              </div>
            ))}
          </div>
          {totalPages > 1 && (
          ...
          <div style={st.modal} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
                {onBackToHouses && (
                  <button onClick={() => { setSelected(null); setEditingField(null); onBackToHouses(); }}
                    style={{ background: 'transparent', border: '1px solid #b8b8bf', color: '#52525b', fontSize: '14px', padding: '5px 10px', borderRadius: '7px', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                    ← Houses
                  </button>
                )}
            ...
            <div style={{ ...st.tabs, position: 'relative' }}>
              ...
              <div ref={moreTabRef} style={{ position: 'relative', display: 'flex', alignItems: 'stretch' }}>
                ...
                {showMoreTabs && (
                  <div style={{ position: 'absolute', top: '100%', right: 0, background: '#ffffff', border: '1px solid #c9c9cf', borderRadius: '10px', overflow: 'hidden', zIndex: 50, minWidth: '140px', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
                    {MORE_TABS.map(t => (
                      <button key={t} onClick={() => { setActiveTab(t); setEditingField(null); setShowMoreTabs(false); }}
                        style={{ display: 'block', width: '100%', textAlign: 'left', padding: '10px 16px', background: activeTab === t ? '#c9c9cf' : 'transparent', border: 'none', color: activeTab === t ? '#18181b' : '#52525b', fontSize: '14px', cursor: 'pointer', borderBottom: '1px solid #c9c9cf' }}
                        onMouseEnter={e => e.currentTarget.style.background = '#c9c9cf'}
                        onMouseLeave={e => e.currentTarget.style.background = activeTab === t ? '#c9c9cf' : 'transparent'}>
                        {t.charAt(0).toUpperCase() + t.slice(1)}
                      </button>
                    ))}
                  </div>
            ...
            <div style={st.modalBody}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                      {clientBalance !== null && (
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px' }}>
                          <span style={{ fontSize: '22px', fontWeight: '700', color: clientBalance > 0 ? '#dc2626' : clientBalance < 0 ? '#16a34a' : '#52525b' }}>
                            {clientBalance > 0 ? `$${clientBalance.toFixed(2)}` : clientBalance < 0 ? `$${Math.abs(clientBalance).toFixed(2)}` : '$0.00'}
                          </span>
                          <span style={{ fontSize: '13px', color: clientBalance > 0 ? '#dc2626' : clientBalance < 0 ? '#16a34a' : '#52525b' }}>
              ...
              {activeTab === 'UAs' && (
                <Card title="UA Records" full action={
                  ...
                  {uaRecords.length === 0 ? <p style={{ color: '#4b5563', fontSize: '14px' }}>No UA records yet.</p> : (
                    <>
                      <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
                        ...
                        <div style={{ background: '#f7f7f9', borderRadius: '8px', padding: '8px 14px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                          ...
                          <span style={{ color: '#52525b', fontSize: '14px' }}>Total</span>
                        </div>
                      </div>
              ...
              {stayDetailModal && (() => {
                ...
                return (
                  <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3500 }}
                    ...
                    <div style={{ background: '#ffffff', borderRadius: '16px', padding: '24px', width: '100%', maxWidth: '560px', maxHeight: '80vh', overflowY: 'auto', border: '1px solid #c9c9cf' }}
                      onClick={e => e.stopPropagation()}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px' }}>
                        <h3 style={{ color: '#18181b', margin: 0, fontSize: '17px' }}>{titles[stayDetailModal.type]}</h3>
                        <button onClick={() => setStayDetailModal(null)} style={{ background: 'none', border: 'none', color: '#4b5563', fontSize: '20px', cursor: 'pointer' }}>×</button>
                      </div>

                      {stayDetailModal.type === 'timeline' && (
                        h.timeline.length === 0 ? <p style={{ color: '#71717a', fontSize: '14px' }}>No timeline entries during this stay.</p> : (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {h.timeline.map(e => (
                              <div key={e.id} style={{ background: '#ffffff', border: '1px solid #d4d4da', borderRadius: '8px', padding: '10px 12px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                  <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: tColors[e.entry_type] || '#52525b', flexShrink: 0 }} />
                                  ...
                                  <span style={{ color: '#71717a', fontSize: '13px', marginLeft: 'auto' }}>{fmtFull(e.created_at)}</span>
                                </div>
                                {e.notes && <p style={{ color: '#52525b', fontSize: '13px', margin: '4px 0 0 0', lineHeight: 1.5 }}>{e.notes}</p>}
                                {e.author && <p style={{ color: '#71717a', fontSize: '13px', margin: '4px 0 0 0' }}>By {e.author}</p>}
                              </div>
                            ))}
                          </div>
                        )
                      ...
                      {stayDetailModal.type === 'checkins' && (
                        h.checkIns.length === 0 ? <p style={{ color: '#71717a', fontSize: '14px' }}>No weekly check-ins during this stay.</p> : (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {h.checkIns.map(e => (
                              <div key={e.id} style={{ background: '#ffffff', border: '1px solid #d4d4da', borderRadius: '8px', padding: '12px 14px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                                  <span style={{ color: '#18181b', fontSize: '14px', fontWeight: '500' }}>Weekly Check-In</span>
                                  <span style={{ color: '#71717a', fontSize: '13px' }}>{fmtFull(e.created_at)}</span>
                                </div>
                      ...
                      {stayDetailModal.type === 'forms' && (
                        h.overnights.length === 0 && !h.welcomePacket ? <p style={{ color: '#71717a', fontSize: '14px' }}>No forms submitted during this stay.</p> : (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {h.welcomePacket && (
                              <div style={{ background: '#ffffff', border: '1px solid #d4d4da', borderRadius: '8px', padding: '10px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ color: '#0d9488', fontSize: '14px', fontWeight: '500' }}>Welcome Packet</span>
                                <span style={{ color: '#71717a', fontSize: '13px' }}>{fmtFull(h.welcomePacket.created_at)}</span>
                              </div>
                            )}
                            {h.overnights.map(r => (
                              <div key={r.id} style={{ background: '#ffffff', border: '1px solid #d4d4da', borderRadius: '8px', padding: '10px 12px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                                  <span style={{ color: '#c2410c', fontSize: '14px', fontWeight: '500' }}>Overnight Pass Request</span>
                                  <span style={{ fontSize: '13px', padding: '2px 8px', borderRadius: '10px', background: r.status === 'approved' ? '#dcfce7' : r.status === 'denied' ? '#fee2e2' : '#c9c9cf', color: r.status === 'approved' ? '#16a34a' : r.status === 'denied' ? '#dc2626' : '#52525b' }}>{r.status || 'pending'}</span>
                                </div>
                                ...
                                {r.reason && <p style={{ color: '#4b5563', fontSize: '13px', margin: 0 }}>Reason: {r.reason}</p>}
                              </div>
                            ))}
                          </div>
                        )
                      ...
                      {stayDetailModal.type === 'balance' && (
                        <>
                          <p style={{ color: '#52525b', fontSize: '13px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 10px' }}>Charges</p>
                          {h.charges.length === 0 ? <p style={{ color: '#71717a', fontSize: '14px', marginBottom: '16px' }}>No charges during this stay.</p> : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '18px' }}>
                              {h.charges.map(c => (
                                <div key={c.id} style={{ background: '#ffffff', border: '1px solid #d4d4da', borderRadius: '8px', padding: '10px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                  <div>
                                    <span style={{ color: '#3f3f46', fontSize: '14px' }}>{c.description || c.charge_type || 'Charge'}</span>
                                    {c.due_date && <span style={{ color: '#71717a', fontSize: '13px', marginLeft: '8px' }}>Due {fmt(c.due_date)}</span>}
                                  </div>
                          ...
                          <p style={{ color: '#52525b', fontSize: '13px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 10px' }}>Payments</p>
                          {h.payments.length === 0 ? <p style={{ color: '#71717a', fontSize: '14px' }}>No payments during this stay.</p> : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                              {h.payments.map(p => (
                                <div key={p.id} style={{ background: '#ffffff', border: '1px solid #d4d4da', borderRadius: '8px', padding: '10px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                  <div>
                                    ...
                                    {p.payment_date && <span style={{ color: '#71717a', fontSize: '13px', marginLeft: '8px' }}>{fmt(p.payment_date)}</span>}
                                    {p.notes && <span style={{ color: '#4b5563', fontSize: '13px', marginLeft: '8px' }}>{p.notes}</span>}
                                  </div>
              ...
              {activeTab === 'meetings' && (
                <Card title="Meeting Records" full>
                  {meetingRecords.length === 0 ? <p style={{ color: '#4b5563', fontSize: '14px' }}>No meeting records yet.</p> : (
                    <>
                      <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
                      ...
                      <p style={{ fontSize: '14px', color: '#52525b', margin: '0 0 12px 0' }}>Goal: 4 meetings per week. Current week expands automatically.</p>
              ...
              {activeTab === 'chores' && (
                <Card title="Chore Rotation" full>
                  {choreRecords.length === 0 ? <p style={{ color: '#4b5563', fontSize: '14px' }}>No chore assignments yet — set up the rotation from the house's Chores tab.</p> : (
                    <>
                      <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
                        <div style={{ background: '#dbeafe', borderRadius: '8px', padding: '8px 14px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                      ...
                      <p style={{ fontSize: '14px', color: '#52525b', margin: '0 0 12px 0' }}>
              ...
              {activeTab === 'timeline' && (
                <>
                    ...
                    <div style={st.miniForm}>
                      ...
                      {(entryType === 'Meeting' || entryType === 'Check-In') && (
                        <div style={{ marginBottom: '12px' }}>
                          ...
                          <button type="button" onClick={dropPin}
                            style={{ ...sf.input, background: entryForm.pinDropped ? '#dcfce7' : '#ffffff', color: entryForm.pinDropped ? '#16a34a' : '#52525b', cursor: 'pointer', textAlign: 'left', border: entryForm.pinDropped ? '1px solid #0d9488' : '1px solid #b0b0b7' }}>
                      ...
                      {entryType === 'Weekly Check-In' && (
                        <>
                          ...
                          <div style={{ marginBottom: '12px' }}>
                            <label style={sf.label}>If you are not paid in full for program fees, enter your plan for your next payment <span style={{ fontWeight: 400, color: '#6b7280' }}>(How much and when)</span></label>
                      ...
                      {['UA', 'Check-In', 'General Note', 'Jobs Applied For', 'Infraction'].includes(entryType) && (
                        <div style={{ marginBottom: '12px' }}>
                          ...
                          }} style={{ color: '#52525b', fontSize: '14px', marginBottom: '8px', display: 'block' }} />
                          {entryForm.photo_preview && (
                            <div style={{ position: 'relative', display: 'inline-block' }}>
                              <img src={entryForm.photo_preview} alt="Preview" style={{ width: '100%', maxHeight: '180px', objectFit: 'cover', borderRadius: '8px', border: '1px solid #b8b8bf' }} />
                  ...
                  {timelineLoading ? (
                    <p style={{ color: '#4b5563', fontSize: '14px' }}>Loading timeline...</p>
                  ) : timeline.length === 0 ? (
                    <p style={{ color: '#4b5563', fontSize: '14px' }}>No timeline entries yet.</p>
                  ) : (
                    <>
                        ...
                        {timeline.map(entry => (
                          <div key={entry.id} style={{ ...st.timelineCard, ...(entry.entry_type === 'Infraction' ? { borderLeft: '3px solid #dc2626', background: '#fee2e2' } : {}) }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <span style={{ color: '#52525b', fontSize: '14px', whiteSpace: 'nowrap' }}>{formatDate(entry.created_at)}</span>
                                <button onClick={() => deleteTimelineEntry(entry.id)} style={{ background: 'transparent', border: '1px solid #b8b8bf', color: '#4b5563', borderRadius: '4px', padding: '2px 8px', fontSize: '14px', cursor: 'pointer' }}>×</button>
                              </div>
                            </div>
                      ...
                      {hasMoreTimeline && (
                        <div style={{ textAlign: 'center', marginTop: '16px' }}>
                          <button onClick={() => fetchTimeline(selected.id, true)} disabled={timelineLoadingMore}
                            style={{ background: 'transparent', border: '1px solid #b8b8bf', color: '#52525b', padding: '8px 20px', borderRadius: '8px', fontSize: '14px', cursor: 'pointer' }}>
                ...
                {showTimelinePDFModal && (
                  <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000 }}
                    ...
                    <div style={{ background: '#ffffff', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '440px', border: '1px solid #c9c9cf' }}
                      onClick={e => e.stopPropagation()}>
                      <h3 style={{ color: '#18181b', margin: '0 0 4px 0', fontSize: '18px' }}>Generate Timeline PDF</h3>
                      <p style={{ color: '#52525b', fontSize: '14px', margin: '0 0 20px 0' }}>Choose a date range and optional event type filter.</p>
                      <p style={{ color: '#52525b', fontSize: '13px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 10px 0' }}>Common Ranges</p>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '20px' }}>
                        {[
                          ...
                          return <button key={opt.label} onClick={() => setPdfRange(p => ({ ...p, startDate: start, endDate: end }))}
                            style={{ padding: '6px 12px', borderRadius: '20px', border: `1px solid ${active ? '#16a34a' : '#b0b0b7'}`, background: active ? '#dcfce7' : 'transparent', color: active ? '#16a34a' : '#52525b', fontSize: '14px', cursor: 'pointer' }}>{opt.label}</button>;
                        })}
                      </div>
                      <label style={{ color: '#52525b', fontSize: '14px', display: 'block', marginBottom: '6px' }}>Start Date</label>
                      <input type="date" value={pdfRange.startDate} onChange={e => setPdfRange(p => ({ ...p, startDate: e.target.value }))}
                        style={{ width: '100%', background: '#f7f7f9', border: '1px solid #b8b8bf', borderRadius: '8px', padding: '8px 12px', color: '#18181b', fontSize: '14px', marginBottom: '14px', boxSizing: 'border-box' }} />
                      <label style={{ color: '#52525b', fontSize: '14px', display: 'block', marginBottom: '6px' }}>End Date <span style={{ color: '#71717a' }}>(optional)</span></label>
                      <input type="date" value={pdfRange.endDate} onChange={e => setPdfRange(p => ({ ...p, endDate: e.target.value }))}
                        style={{ width: '100%', background: '#f7f7f9', border: '1px solid #b8b8bf', borderRadius: '8px', padding: '8px 12px', color: '#18181b', fontSize: '14px', marginBottom: '14px', boxSizing: 'border-box' }} />
                      <label style={{ color: '#52525b', fontSize: '14px', display: 'block', marginBottom: '6px' }}>Event Type <span style={{ color: '#71717a' }}>(optional)</span></label>
                      <select value={pdfRange.eventType} onChange={e => setPdfRange(p => ({ ...p, eventType: e.target.value }))}
                        style={{ width: '100%', background: '#f7f7f9', border: '1px solid #b8b8bf', borderRadius: '8px', padding: '8px 12px', color: '#18181b', fontSize: '14px', marginBottom: '20px', boxSizing: 'border-box' }}>
              ...
              {activeTab === 'stays' && (
                <Card title="Stay History" full>
                  {staysLoading ? (
                    <p style={{ color: '#4b5563', fontSize: '14px' }}>Loading stay history...</p>
                  ) : stays.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '30px 0' }}>
                      <p style={{ color: '#52525b', fontSize: '14px', margin: 0 }}>No previous stays recorded.</p>
                      <p style={{ color: '#4b5563', fontSize: '14px', margin: '6px 0 0 0' }}>Stay history is saved automatically when a client is discharged.</p>
                    </div>
                  ) : (
                    <>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {stays.map((stay, i) => {
                          ...
                          const balance = parseFloat(stay.balance_at_discharge) || 0;
                          return (
                            <div key={stay.id} style={{ background: '#ffffff', borderRadius: '12px', border: '1px solid #c9c9cf', overflow: 'hidden' }}>
                              <div
                                onClick={() => {
                                ...
                                style={{ padding: '12px 16px', borderBottom: '1px solid #c9c9cf', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                  <span style={{ color: '#71717a', fontSize: '13px', transform: expandedStay === stay.id ? 'rotate(0deg)' : 'rotate(-90deg)', display: 'inline-block', transition: 'transform 0.2s' }}>▾</span>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }} onClick={e => e.stopPropagation()}>
                                  {lengthDays !== null && <span style={{ fontSize: '14px', color: '#52525b' }}>{lengthDays} day{lengthDays !== 1 ? 's' : ''}</span>}
                                  <button
                                    onClick={() => {
                                      const img = new Image();
                                      img.crossOrigin = 'anonymous';
                              ...
                              <div style={{ padding: '12px 16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                                <div>
                                  <p style={{ fontSize: '14px', color: '#52525b', margin: '0 0 3px 0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Move-in</p>
                                  <p style={{ fontSize: '14px', color: '#3f3f46', margin: 0 }}>{formatDateFull(stay.start_date)}</p>
                                </div>
                                <div>
                                  <p style={{ fontSize: '14px', color: '#52525b', margin: '0 0 3px 0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Discharge</p>
                                  <p style={{ fontSize: '14px', color: '#3f3f46', margin: 0 }}>{formatDateFull(stay.discharge_date)}</p>
                                </div>
                                <div>
                                  <p style={{ fontSize: '14px', color: '#52525b', margin: '0 0 3px 0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Reason</p>
                                  <p style={{ fontSize: '14px', color: '#3f3f46', margin: 0 }}>{stay.discharge_reason || '—'}</p>
                                </div>
                                <div>
                                  <p style={{ fontSize: '14px', color: '#52525b', margin: '0 0 3px 0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Balance at discharge</p>
                                  <p style={{ fontSize: '14px', fontWeight: '600', margin: 0, color: balance > 0 ? '#dc2626' : balance < 0 ? '#16a34a' : '#52525b' }}>
                                    {balance > 0 ? `$${balance.toFixed(2)} owed` : balance < 0 ? `$${Math.abs(balance).toFixed(2)} credit` : '$0.00'}
                                  </p>
                                </div>
                                {stay.discharge_notes && (
                                  <div style={{ gridColumn: 'span 2' }}>
                                    <p style={{ fontSize: '14px', color: '#52525b', margin: '0 0 3px 0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Notes</p>
                                    <p style={{ fontSize: '14px', color: '#52525b', margin: 0, lineHeight: 1.5 }}>{stay.discharge_notes}</p>
                                  </div>
                                )}
                                {stay.discharged_by && (
                                  <div style={{ gridColumn: 'span 2' }}>
                                    <p style={{ fontSize: '14px', color: '#4b5563', margin: 0 }}>Discharged by {stay.discharged_by}</p>
                                  </div>
                                )}
                              ...
                              {expandedStay === stay.id && (() => {
                                ...
                                if (loading) return <div style={{ padding: '16px', borderTop: '1px solid #c9c9cf', color: '#4b5563', fontSize: '14px' }}>Loading history...</div>;
                                if (!h) return null;
                                ...
                                return (
                                  <div style={{ borderTop: '1px solid #c9c9cf', padding: '16px', background: '#ffffff' }}>
                                    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '20px' }}>
                                      {[
                                        ...
                                        <div key={s.label} onClick={() => setStayDetailModal({ stayId: stay.id, type: s.modal })}
                                          style={{ background: '#ffffff', border: '1px solid #d4d4da', borderRadius: '8px', padding: '8px 12px', cursor: 'pointer' }}
                                          onMouseEnter={e => e.currentTarget.style.borderColor = '#b0b0b7'}
                                          onMouseLeave={e => e.currentTarget.style.borderColor = '#d4d4da'}>
                                          <div style={{ color: s.color, fontSize: '15px', fontWeight: '700' }}>{s.val}</div>
                                          <div style={{ color: '#71717a', fontSize: '12px' }}>{s.label}</div>
                                        </div>
                                      ))}
                                    </div>

                                    {noHistory && <p style={{ color: '#71717a', fontSize: '14px', textAlign: 'center', padding: '16px 0' }}>No history found for this stay.</p>}
                                  </div>
                                );
                              })()}
                            </div>
              ...
              {activeTab === 'notes' && (
                <Card title="Staff notes" full>
                  <textarea defaultValue={selected.client_notes || ''} onBlur={saveNotes} placeholder="Add staff notes here..."
                    style={{ width: '100%', backgroundColor: '#ffffff', border: '1px solid #b8b8bf', borderRadius: '8px', padding: '12px 14px', color: '#18181b', fontSize: '14px', lineHeight: '1.6', resize: 'vertical', boxSizing: 'border-box', minHeight: '220px', outline: 'none', fontFamily: "'Inter', 'system-ui', sans-serif" }} />
                  <p style={{ color: '#52525b', fontSize: '14px', marginTop: '8px' }}>Changes save automatically when you click away.</p>
                </Card>
              )}
              ...
              {isAdmin && (
                <div style={{ padding: '20px 0 8px', display: 'flex', justifyContent: 'center' }}>
                  <button onClick={() => deleteClient(selected)}
                    style={{ background: 'transparent', border: '1px solid #fee2e2', color: '#71717a', fontSize: '12px', padding: '4px 12px', borderRadius: '6px', cursor: 'pointer', letterSpacing: '0.5px' }}>
                    🗑 Delete Client
                  </button>
                </div>
              )}
        ...
        <div style={{ ...st.overlay, zIndex: 2000 }} onClick={() => setStatusModal(null)}>
          <div style={{ ...st.modal, maxWidth: '420px', marginTop: '120px' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #c9c9cf' }}>
              ...
              <p style={{ color: '#4b5563', fontSize: '14px', margin: '4px 0 0 0' }}>{statusModal.client.full_name}</p>
            </div>
            <div style={{ padding: '20px 24px' }}>
              ...
              {statusModal.newStatus === 'Discharged' && (
                <>
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginTop: '6px' }}>
                      {['yes', 'no'].map(v => (
                        <button key={v} onClick={() => setStatusForm(p => ({ ...p, successful_discharge: v }))}
                          style={{ flex: 1, padding: '9px', borderRadius: '8px', border: '1px solid', cursor: 'pointer', fontSize: '14px', fontWeight: '500',
                            borderColor: statusForm.successful_discharge === v ? (v === 'yes' ? '#16a34a' : '#dc2626') : '#4b5563',
                            background: statusForm.successful_discharge === v ? (v === 'yes' ? '#dcfce7' : '#fee2e2') : 'transparent',
                            color: statusForm.successful_discharge === v ? (v === 'yes' ? '#16a34a' : '#dc2626') : '#52525b',
                          }}>
                          {v === 'yes' ? 'Yes' : 'No'}
                        </button>
                      ))}
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginTop: '6px' }}>
                      {['Complete', 'Incomplete'].map(v => (
                        <button key={v} onClick={() => setStatusForm(p => ({ ...p, discharge_type: v }))}
                          style={{ flex: 1, padding: '9px', borderRadius: '8px', border: '1px solid', cursor: 'pointer', fontSize: '14px', fontWeight: '500',
                            borderColor: statusForm.discharge_type === v ? '#2563eb' : '#4b5563',
                            background: statusForm.discharge_type === v ? '#dbeafe' : 'transparent',
                            color: statusForm.discharge_type === v ? '#2563eb' : '#52525b',
                          }}>
                          {v}
                        </button>
                      ))}
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginTop: '6px' }}>
                      {['Positive', 'Negative', 'N/A'].map(v => (
                        <button key={v} onClick={() => setStatusForm(p => ({ ...p, ua_at_discharge: v }))}
                          style={{ flex: 1, padding: '9px', borderRadius: '8px', border: '1px solid', cursor: 'pointer', fontSize: '14px', fontWeight: '500',
                            borderColor: statusForm.ua_at_discharge === v ? '#2563eb' : '#4b5563',
                            background: statusForm.ua_at_discharge === v ? '#dbeafe' : 'transparent',
                            color: statusForm.ua_at_discharge === v ? '#2563eb' : '#52525b',
                          }}>
                          {v}
                        </button>
                      ))}
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginTop: '6px' }}>
                      {['Yes', 'No'].map(v => (
                        <button key={v} onClick={() => setStatusForm(p => ({ ...p, two_week_notice: v }))}
                          style={{ flex: 1, padding: '9px', borderRadius: '8px', border: '1px solid', cursor: 'pointer', fontSize: '14px', fontWeight: '500',
                            borderColor: statusForm.two_week_notice === v ? '#2563eb' : '#4b5563',
                            background: statusForm.two_week_notice === v ? '#dbeafe' : 'transparent',
                            color: statusForm.two_week_notice === v ? '#2563eb' : '#52525b',
                          }}>
                          {v}
                        </button>
                      ))}
                  ...
                  <div style={{ marginBottom: '12px', padding: '12px 14px', background: '#ffffff', borderRadius: '8px', border: '1px solid #d4d4da' }}>
                    <p style={{ margin: '0 0 10px', fontSize: '13px', color: '#71717a', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Flags</p>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', marginBottom: '10px' }}>
                      <input type="checkbox" checked={statusForm.not_allowed_back || false}
                        ...
                        style={{ width: '16px', height: '16px', accentColor: '#b22222', cursor: 'pointer' }} />
                      <span style={{ fontSize: '14px', color: statusForm.not_allowed_back ? '#dc2626' : '#52525b', fontWeight: statusForm.not_allowed_back ? '600' : '400' }}>
                        🚫 Not allowed back
                      </span>
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                      ...
                      <span style={{ fontSize: '14px', color: statusForm.needs_review_before_readmit ? '#c2410c' : '#52525b', fontWeight: statusForm.needs_review_before_readmit ? '600' : '400' }}>
              ...
              {!['Waiting List', 'Pending', 'Active', 'Discharged'].includes(statusModal.newStatus) && (
                <p style={{ color: '#52525b', fontSize: '14px', margin: '0 0 16px 0' }}>This will update the client's status to {statusModal.newStatus}.</p>
              )}
...
function Card({ title, children, full, action }) {
  return (
    <div style={{ background: '#f7f7f9', border: '1px solid #c9c9cf', borderRadius: '12px', padding: '14px 16px', gridColumn: full ? '1 / -1' : undefined }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        <p style={{ fontSize: '14px', fontWeight: '500', color: '#4b5563', textTransform: 'uppercase', letterSpacing: '0.05em', margin: 0 }}>{title}</p>
        {action && action}
      </div>
      {children}
    </div>
...
const st = {
  ...
  search: { width: '100%', maxWidth: '360px', backgroundColor: '#ffffff', border: '1px solid #b8b8bf', borderRadius: '8px', padding: '10px 14px', color: '#18181b', fontSize: '14px' },
  viewToggleWrap: { display: 'flex', gap: '8px', flexWrap: 'wrap' },
  filters: { display: 'flex', gap: '8px', flexWrap: 'wrap' },
  filterBtn: { padding: '6px 14px', borderRadius: '20px', border: '1px solid #b8b8bf', background: 'transparent', color: '#52525b', fontSize: '14px', cursor: 'pointer' },
  filterActive: { background: '#b22222', borderColor: '#b22222', color: '#18181b' },
  table: { background: '#f7f7f9', borderRadius: '12px', overflow: 'hidden', border: '1px solid #c9c9cf' },
  tableHeader: { display: 'flex', padding: '12px 16px', borderBottom: '2px solid #fee2e2', fontSize: '14px', color: '#4b5563', fontWeight: '500', textTransform: 'uppercase', letterSpacing: '0.05em' },
  row: { display: 'flex', alignItems: 'center', padding: '14px 16px', borderBottom: '1px solid #cfcfd4', cursor: 'pointer' },
  ...
  modal: { background: '#ffffff', borderRadius: '16px', border: '1px solid #c9c9cf', width: '100%', maxWidth: '860px', overflow: 'hidden' },
  ...
  modalSub: { fontSize: '14px', color: '#4b5563', margin: '2px 0 0 0' },
  badges: { display: 'flex', gap: '6px', flexWrap: 'wrap', marginTop: '6px', alignItems: 'center' },
  closeBtn: { width: '30px', height: '30px', borderRadius: '50%', border: '1px solid #b8b8bf', background: 'transparent', cursor: 'pointer', color: '#52525b', fontSize: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  tabs: { display: 'flex', borderBottom: '1px solid #c9c9cf', padding: '0 20px', overflowX: 'visible' },
  tab: { padding: '10px 14px', fontSize: '14px', cursor: 'pointer', color: '#4b5563', background: 'transparent', border: 'none', borderBottom: '2px solid transparent', whiteSpace: 'nowrap', flexShrink: 0 },
  ...
  pageBtn: { padding: '6px 12px', borderRadius: '8px', border: '1px solid #b8b8bf', background: 'transparent', color: '#52525b', fontSize: '14px', cursor: 'pointer', transition: 'all 0.15s' },
  ...
  ellipsis: { color: '#52525b', fontSize: '14px', padding: '0 4px' },
