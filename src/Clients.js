...
function Clients() {
  ...
  const fetchClients = useCallback(async () => {
    ...
    try {
      ...
      if (countError) {
        ...
        alert('There was a problem loading the client count.');
        setClients([]);
        setTotalCount(0);
        return;
      }
      ...
      if (dataError) {
        console.error('Error fetching clients:', dataError);
        alert('There was a problem loading clients.');
        setClients([]);
        return;
      }
      ...
      alert('Something went wrong while loading clients.');
      setClients([]);
      setTotalCount(0);
    } finally {
      setLoading(false);
  ...
  const fetchHouses = useCallback(async () => {
    try {
      ...
      console.error('Unexpected fetchHouses error:', error);
    }
  }, []);
  ...
  const fetchTimeline = async (clientId) => {
    try {
      ...
      if (error) {
        ...
        alert('There was a problem loading the timeline.');
        return;
      }
      ...
      all.forEach(async (entry) => {
        if (entry.latitude && entry.longitude) {
          ...
          if (address) {
            setLocationLabels((prev) => ({ ...prev, [entry.id]: address }));
          }
        }
      });
    } catch (error) {
      console.error('Unexpected fetchTimeline error:', error);
    }
  };
  ...
  const confirmStatusChange = async () => {
    ...
    if (newStatus === 'Waiting List') {
      ...
      if (wlError) {
        alert('Error adding to waiting list: ' + wlError.message);
        return;
      }
    }

    if (newStatus === 'Pending') {
      if (!statusForm.house_id) {
    ...
    const { error } = await supabase.from('client_timeline').insert([
      {
        ...
        event_name:
          entryType === 'UA'
            ? entryForm.ua_result
            : entryType === 'Chores'
              ? entryForm.chore_status
              : null,
        meeting_name:
          entryType === 'Meeting'
            ? entryForm.meeting_name
            : entryType === 'Chores'
              ? entryForm.chore_name
              : null,
  ...
  const saveNotes = async (e) => {
    ...
    setClients((prev) =>
      prev.map((c) => (c.id === selected.id ? { ...c, client_notes: newNotes } : c))
    );
  };
        ...
        {isEditing ? (
          options ? (
            <select
              ...
              onChange={(e) => setEditingField((p) => ({ ...p, value: e.target.value }))}
              onBlur={saveField}
              style={{
                background: '#111',
          ...
          <span
            ...
            style={{
              fontSize: '13px',
              color: isAlert ? '#f87171' : value ? '#ddd' : '#444',
              textAlign: 'right',
    ...
    return (
      <div
        style={{
          background: '#1a1a1a',
          borderRadius: '10px',
          border: `1px solid ${isThisWeek ? '#2a3d52' : '#333'}`,
          marginBottom: '10px',
          overflow: 'hidden',
        }}
      >
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  {m.latitude && m.longitude && (
                    <LocationPin entryId={m.id} lat={m.latitude} lng={m.longitude} />
                  )}
                  {m.notes && (
                    <p
                      style={{
    ...
    return (
      <div
        style={{
          background: '#1a1a1a',
          borderRadius: '10px',
          border: `1px solid ${isThisWeek ? '#1a3a2a' : '#333'}`,
          marginBottom: '10px',
          overflow: 'hidden',
        }}
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span
              style={{
                color: allDone ? '#4ade80' : notCompleted > 0 ? '#f87171' : '#fb923c',
                fontSize: '11px',
              }}
            >
              {allDone ? '✓' : '✗'}
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1, flexWrap: 'wrap' }}>
                    <span style={{ ...st.badge, background: col.bg, color: col.color, fontSize: '11px' }}>
                      {c.event_name}
                    </span>
                    {c.meeting_name && (
                      <span style={{ fontSize: '13px', color: '#ddd', fontWeight: '500' }}>
  ...
  return (
    <div style={st.page}>
        ...
        <div style={st.filters}>
          {statusFilters.map((f) => (
            <button
              ...
              style={{ ...st.filterBtn, ...(statusFilter === f ? st.filterActive : {}) }}
            >
              {f}
            </button>
          ))}
            ...
            <div style={st.pagination}>
              ...
              <div style={st.pageNumbers}>
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((p, i) =>
                    p === '...' ? (
                      <span key={`ellipsis-${i}`} style={st.ellipsis}>
                        …
                      </span>
                    ) : (
                      <button
                        ...
                        style={{ ...st.pageBtn, ...(currentPage === p ? st.pageBtnActive : {}) }}
                      >
                        {p}
                      </button>
                    )
        ...
        >
          <div style={st.modal} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={st.badges}>
                  <span
                    style={{
                      ...
                      color: statusColor(selected.status).color,
                    }}
                  >
                  ...
                  <select
                    ...
                    onChange={(e) => updateLevel(selected.id, parseInt(e.target.value))}
                  ...
                  {selected.sor_grant && (
                    <span style={{ ...st.badge, background: '#3a2d1e', color: '#fb923c' }}>
                      SOR grant
                    </span>
                  )}
                </div>
                ...
                {hasFullAccess && STATUS_FLOW[selected.status]?.length > 0 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '8px' }}>
                    ...
                    <select
                      defaultValue=""
                      onChange={(e) => {
                        if (e.target.value) openStatusModal(selected, e.target.value);
                        e.target.value = '';
                      }}
                      style={{
            ...
            <div style={st.modalBody}>
                  ...
                  <div style={st.grid}>
                    <Card title="Contact info">
                      ...
                      <EditableField
                        ...
                        options={['Male', 'Female', 'Non-binary', 'No Response']}
                      />
                      <EditableField
                        label="Ethnicity"
                        field="ethnicity"
                        value={selected.ethnicity}
                        options={[
                    ...
                    <Card title="House assignment">
                      ...
                      <EditableField
                        ...
                        options={['Single', 'Double', 'Houseperson']}
                      />
                    ...
                    <Card title="PO & legal">
                      ...
                      <EditableField
                        ...
                        value={selected.personal_status}
                        alert={selected.personal_status === 'Currently Incarcerated'}
                        options={[
                          'Currently Incarcerated',
                          'Homeless',
                          'Housing Insecure',
                          'Currently staying at Inpatient Treatment',
                          'Currently being referred by Recovery Community Center',
                        ]}
                      />
                      <EditableField
                        label="Sex offense"
                        field="sex_offender"
                        value={selected.sex_offender}
                        options={['Yes', 'No']}
                      />
                      <EditableField
                        label="On probation"
                        field="on_probation"
                        value={selected.on_probation}
                        options={['Yes', 'No']}
                      />
                      <EditableField
                        label="On parole"
                        field="on_parole"
                        value={selected.on_parole}
                        options={['Yes', 'No']}
                      />
                    </Card>
                    ...
                    <Card title="Sponsor">
                      ...
                      <EditableField
                        label="Sponsor phone"
                        field="sponsor_phone"
                        value={selected.sponsor_phone}
                      />
                      <EditableField
                        label="Recovery meetings"
                        field="recovery_meetings"
                        value={selected.recovery_meetings}
                        options={['AA', 'NA', 'Both AA & NA', 'Smart Recovery', 'Other', 'None']}
                      />
                    </Card>
                    ...
                    <Card title="Recovery">
                      <EditableField
                        ...
                        field="substance_history"
                        value={selected.substance_history}
                        options={['Yes', 'No']}
                      />
                      <EditableField
                        label="Drug of choice"
                        field="drug_of_choice"
                        value={selected.drug_of_choice}
                      />
                      <EditableField
                        label="Sober date"
                        field="sober_date"
                        value={selected.sober_date}
                      />
                      <EditableField
                        label="Treatment history"
                        field="treatment_history"
                        value={selected.treatment_history}
                        options={['Yes', 'No']}
                      />
                      <EditableField
                        label="OUD"
                        ...
                        {['Negative', 'Positive', 'Inconclusive', 'Refused'].map((result) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span style={{ color: col.color, fontSize: '11px', opacity: 0.8 }}>
                                {result}
                              </span>
                            </div>
                          );
                    ...
                    <>
                      <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
                        ...
                        >
                          ...
                          <span style={{ color: '#60a5fa', fontSize: '11px', opacity: 0.8 }}>
                            Total Meetings
                          </span>
                        </div>
                      </div>

                      <p style={{ fontSize: '11px', color: '#555', margin: '0 0 12px 0' }}>
                        Goal: 4 meetings per week. Current week expands automatically.
                      </p>
                ...
                <Card title="Chore Records" full>
                  {choreRecords.length === 0 ? (
                    ...
                    <>
                      ...
                      <p style={{ fontSize: '11px', color: '#555', margin: '0 0 12px 0' }}>
                        Current week expands automatically. ✓ = all chores completed that week.
                      </p>
                      ...
                      try {
                        ...
                        return meds.length > 0 ? (
                                ...
                                >
                                  {med.dosage && (
                                    <div>
                                      ...
                                      <p style={{ color: '#ddd', fontSize: '13px', margin: '2px 0 0 0' }}>
                                        {med.dosage}
                                      </p>
                                    </div>
                                  )}
                          ...
                          <p style={{ color: '#666', fontSize: '14px' }}>
                            No medications listed on application.
                          </p>
                        );
                      } catch {
                        return (
                    ...
                    <div style={st.miniForm}>
                            ...
                            {['Positive', 'Negative', 'Inconclusive', 'Refused'].map((opt) => (
                              ...
                              >
                                <input
                                  ...
                                  name="ua_result"
                                  value={opt}
                                  checked={entryForm.ua_result === opt}
                                  onChange={() => setEntryForm((p) => ({ ...p, ua_result: opt }))}
                                />
                                {opt}
                              </label>
                            ))}
                      ...
                      {entryType === 'Meeting' && (
                        <div style={{ marginBottom: '12px' }}>
                          ...
                          <input
                            value={entryForm.meeting_name}
                            onChange={(e) =>
                              setEntryForm((p) => ({ ...p, meeting_name: e.target.value }))
                            }
                            style={sf.input}
                            placeholder="e.g. New Beginnings, Ground Zero"
                          />
                        </div>
                      )}
                      ...
                      {entryType === 'Chores' && (
                        <>
                          <div style={{ marginBottom: '12px' }}>
                            ...
                            <input
                              value={entryForm.chore_name}
                              onChange={(e) =>
                                setEntryForm((p) => ({ ...p, chore_name: e.target.value }))
                              }
                              style={sf.input}
                              placeholder="e.g. Kitchen, Bathroom, Yard"
                            />
                          </div>
                              ...
                              {['Completed', 'Not Completed', 'Partial'].map((opt) => (
                                ...
                                >
                                  <input
                                    ...
                                    name="chore_status"
                                    value={opt}
                                    checked={entryForm.chore_status === opt}
                                    onChange={() =>
                                      setEntryForm((p) => ({ ...p, chore_status: opt }))
                                    }
                                  />
                                  {opt}
                                </label>
                      ...
                      {entryType === 'Mood Check-In' && (
                        <div style={{ marginBottom: '12px' }}>
                          <label style={sf.label}>
                            Mood Value (1–10): {entryForm.mood_value}
                          </label>
                          <input
                            type="range"
                            min="1"
                            max="10"
                            value={entryForm.mood_value}
                            onChange={(e) =>
                              setEntryForm((p) => ({ ...p, mood_value: e.target.value }))
                            }
                            style={{ width: '100%' }}
                          />
                        </div>
                            ...
                            {['Here', 'Not Here'].map((opt) => (
                              ...
                              >
                                <input
                                  ...
                                  name="checkin"
                                  value={opt}
                                  checked={entryForm.checkin_status === opt}
                                  onChange={() =>
                                    setEntryForm((p) => ({ ...p, checkin_status: opt }))
                                  }
                                />
                                {opt}
                              </label>
                      ...
                      {(entryType === 'Meeting' || entryType === 'Check-In') && (
                        <div style={{ marginBottom: '12px' }}>
                          ...
                          <button
                            ...
                            style={{
                              ...sf.input,
                              background: entryForm.pinDropped ? '#1e3a2f' : '#1a1a1a',
                              color: entryForm.pinDropped ? '#4ade80' : '#aaa',
                              cursor: 'pointer',
                              textAlign: 'left',
                              border: entryForm.pinDropped
                                ? '1px solid #1D9E75'
                                : '1px solid #444',
                            }}
                          >
                            {entryForm.pinDropped
                              ? '📍 Pin dropped'
                              : '📍 Drop pin (uses your current location)'}
                          </button>
                        </div>
                      )}
                      ...
                      <div style={{ marginBottom: '12px' }}>
                        ...
                        <input
                          ...
                          onChange={(e) => setEntryForm((p) => ({ ...p, author: e.target.value }))}
                          style={sf.input}
                      ...
                      <button onClick={saveTimelineEntry} style={sf.confirmBtn}>
                        Save Entry
                      </button>
                    </div>
                  )}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      {timeline.map((entry) => (
                        <div key={entry.id} style={st.timelineCard}>
                            ...
                            >
                              <div
                                style={{
                                  ...
                                  height: '10px',
                                  borderRadius: '50%',
                                  background: entryColor(entry.entry_type),
                                  flexShrink: 0,
                                }}
                              />
                          ...
                          <p style={{ color: '#555', fontSize: '11px', margin: '6px 0 0 0' }}>
                            By {entry.author}
                          </p>
                        </div>
                      ))}
              ...
              {activeTab === 'documents' && (
                <Card title="Documents" full>
                  <p style={{ color: '#666', fontSize: '14px' }}>
                    Documents will appear here once file uploads are set up.
                  </p>
                </Card>
              )}
            </div>
      ...
      {statusModal && (
        <div
          style={{ ...st.overlay, zIndex: 2000 }}
          onClick={() => setStatusModal(null)}
        >
          <div
            style={{ ...st.modal, maxWidth: '420px', marginTop: '120px' }}
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div style={{ padding: '20px 24px' }}>
              {statusModal.newStatus === 'Waiting List' && (
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <select
                    value={statusForm.list_type}
                    onChange={(e) =>
                      setStatusForm((p) => ({ ...p, list_type: e.target.value }))
                    }
                    style={sf.input}
                  >
                    {LISTS.map((l) => (
              ...
              {statusModal.newStatus === 'Pending' && (
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <select
                    value={statusForm.house_id}
                    onChange={(e) =>
                      setStatusForm((p) => ({ ...p, house_id: e.target.value }))
                    }
                    style={sf.input}
                  >
              ...
              {statusModal.newStatus === 'Active' && (
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <input
                    type="date"
                    value={statusForm.move_in_date}
                    onChange={(e) =>
                      setStatusForm((p) => ({ ...p, move_in_date: e.target.value }))
                    }
                    style={sf.input}
                  />
                </div>
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                <button onClick={() => setStatusModal(null)} style={sf.cancelBtn}>
                  Cancel
                </button>
                <button onClick={confirmStatusChange} style={sf.confirmBtn}>
                  Confirm
                </button>
              </div>
            </div>
