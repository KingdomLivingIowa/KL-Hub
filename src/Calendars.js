...
const RECURRENCE_OPTIONS = ['none', 'weekly', 'biweekly', 'monthly'];

// ─── Shared styles ────────────────────────────────────────────────────────────
const s = {
  tabBtn: (active) => ({
    padding: '8px 18px', borderRadius: 8, border: '1px solid #444', cursor: 'pointer', fontSize: 13,
    background: active ? '#2a2a2a' : 'transparent', color: active ? '#fff' : '#888', fontWeight: active ? 600 : 400,
  }),
        ...
        {cells.map((day, i) => {
          ...
          return (
            <div key={ymd} onClick={() => onDayClick && onDayClick(ymd, events)}
              style={{ minHeight: 64, background: isToday ? '#2e1a1a' : '#1e1e1e', borderRadius: 8, padding: '6px 8px', cursor: onDayClick ? 'pointer' : 'default', border: isToday ? '1px solid #b22222' : '1px solid #2a2a2a', transition: 'background 0.15s' }}>
              ...
              {events.slice(0, 3).map((ev, j) => renderDot ? renderDot(ev, j) : (
                <div key={j} style={{ fontSize: 10, color: '#fff', background: ev.color || '#b22222', borderRadius: 3, padding: '1px 4px', marginBottom: 2, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                  {ev.label}
                </div>
              ))}
...
function MoveInCalendar() {
  ...
  const fetchClients = useCallback(async () => {
    let query = supabase.from('clients')
      .select('id, full_name, start_date, house_id, houses(name), gender, status')
  ...
  // Build eventsByDate — show all clients with a start_date (upcoming and past)
  const eventsByDate = {};
  clients.forEach(c => {
    if (!c.start_date) return;
    if (!eventsByDate[c.start_date]) eventsByDate[c.start_date] = [];
    eventsByDate[c.start_date].push({
      label: c.full_name,
      house: c.houses?.name || '',
      gender: c.gender,
      status: c.status,
  ...
  const handleDayClick = (ymd, events) => {
  ...
  return (
    <div>
      <p style={{ fontSize: 13, color: '#666', marginBottom: 20 }}>
        Clients with a move-in date appear here automatically. Set a client's start date in their profile to add them.
      </p>
      <div style={s.card}>
        <CalendarGrid
...
function HouseEventsCalendar() {
  ...
  const [selectedDay, setSelectedDay] = useState(null);
  const [selectedEvents, setSelectedEvents] = useState([]);
  const [form, setForm] = useState({ title: '', description: '', event_date: '', start_time: '', end_time: '', house_id: '', is_recurring: false, recurrence: 'none', recurrence_day: 1 });
  const [saving, setSaving] = useState(false);

  const fetchHouses = useCallback(async () => {
    let query = supabase.from('houses').select('id, name').order('name');
    if (isHouseManagerRole && assignedHouseIds.length > 0)
      query = query.in('id', assignedHouseIds);
    const { data } = await query;
    setHouses(data || []);
    if (data && data.length > 0 && !form.house_id) setForm(f => ({ ...f, house_id: data[0].id }));
  }, [isHouseManagerRole, assignedHouseIds]); // eslint-disable-line react-hooks/exhaustive-deps

  const fetchEvents = useCallback(async () => {
    let query = supabase.from('house_events').select('*, houses(name)').order('event_date');
    if (isHouseManagerRole && assignedHouseIds.length > 0)
      query = query.in('house_id', assignedHouseIds);
    const { data } = await query;
    setEvents(data || []);
  ...
  useEffect(() => { fetchHouses(); fetchEvents(); }, [fetchHouses, fetchEvents]);
  ...
  events.forEach(ev => {
    if (houseFilter !== 'all' && ev.house_id !== houseFilter) return;

    if (!ev.is_recurring || ev.recurrence === 'none') {
      if (!expandedEvents[ev.event_date]) expandedEvents[ev.event_date] = [];
      expandedEvents[ev.event_date].push({ ...ev, label: ev.title });
    } else {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (ev.recurrence === 'weekly' && date.getDay() === originDate.getDay()) {
          if (!expandedEvents[ymd]) expandedEvents[ymd] = [];
          expandedEvents[ymd].push({ ...ev, label: ev.title });
        ...
        } else if (ev.recurrence === 'monthly' && date.getDate() === originDate.getDate()) {
          if (!expandedEvents[ymd]) expandedEvents[ymd] = [];
          expandedEvents[ymd].push({ ...ev, label: ev.title });
        }
      }
  ...
  const saveEvent = async () => {
    if (!form.title || !form.event_date || !form.house_id) return alert('Title, date, and house are required.');
    setSaving(true);
    await supabase.from('house_events').insert([{
      ...
      house_id: form.house_id, is_recurring: form.is_recurring,
      recurrence: form.is_recurring ? form.recurrence : 'none',
      created_by: user?.id,
    }]);
    setSaving(false);
    setShowAddModal(false);
    setForm(f => ({ ...f, title: '', description: '', event_date: '', start_time: '', end_time: '' }));
    fetchEvents();
  };
  ...
  const EVT_COLORS = ['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899', '#b22222'];
  const houseColorMap = {};
  houses.forEach((h, i) => { houseColorMap[h.id] = EVT_COLORS[i % EVT_COLORS.length]; });
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <button style={s.btn()} onClick={() => setShowAddModal(true)}>+ Add Event</button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 16 }}>
        {houses.map(h => (
          <div key={h.id} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <div style={{ width: 10, height: 10, borderRadius: 2, background: houseColorMap[h.id] }} />
            <span style={{ fontSize: 12, color: '#888' }}>{h.name}</span>
          </div>
        ))}
      </div>
      ...
      <div style={s.card}>
        <CalendarGrid
          year={year} month={month}
          eventsByDate={expandedEvents}
          onDayClick={(ymd, evs) => { if (evs.length) { setSelectedDay(ymd); setSelectedEvents(evs); } }}
          onPrev={() => { if (month === 0) { setMonth(11); setYear(y => y - 1); } else setMonth(m => m - 1); }}
          onNext={() => { if (month === 11) { setMonth(0); setYear(y => y + 1); } else setMonth(m => m + 1); }}
          renderDot={(ev, j) => (
            <div key={j} style={{ fontSize: 10, color: '#fff', background: houseColorMap[ev.house_id] || '#b22222', borderRadius: 3, padding: '1px 4px', marginBottom: 2, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
              {ev.label}
            </div>
          )}
        />
            ...
            <div key={i} style={{ padding: '12px 0', borderBottom: '1px solid #333' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>{ev.houses?.name}</div>
                ...
                {(hasFullAccess || ev.created_by === user?.id) && (
        ...
        <Modal title="Add House Event" onClose={() => setShowAddModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              ...
              <select value={form.house_id} onChange={e => setForm(f => ({ ...f, house_id: e.target.value }))} style={s.select}>
            ...
            <div>
              ...
              <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} style={s.input} placeholder="e.g. House Meeting" />
            </div>
            <div>
            ...
            {form.is_recurring && (
              <div>
                ...
                <select value={form.recurrence} onChange={e => setForm(f => ({ ...f, recurrence: e.target.value }))} style={s.select}>
                  {RECURRENCE_OPTIONS.filter(o => o !== 'none').map(o => <option key={o} value={o}>{o.charAt(0).toUpperCase() + o.slice(1)}</option>)}
                </select>
              </div>
            )}
...
function VacationCalendar() {
  ...
  // Only approved vacations show on calendar
  const eventsByDate = {};
  requests.filter(r => r.status === 'approved').forEach(r => {
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      ...
      eventsByDate[ymd].push({ label: r.user_name || r.user_id, ...r });
    }
  });
  ...
  const submitRequest = async () => {
    ...
    // Notify all upper management / admins
  ...
  const reviewRequest = async (id, status, requestUserId, requestUserName, startDate, endDate) => {
  ...
  return (
    <div>
      ...
      {/* All requests (for approvers) */}
      {activeView === 'requests' && (
        <div style={s.card}>
          <div style={{ fontSize: 13, color: '#666', marginBottom: 16 }}>
            {canApprove ? 'All submitted vacation requests' : 'You do not have permission to review requests.'}
          </div>
            ...
            <div key={r.id} style={{ padding: '14px 0', borderBottom: '1px solid #333' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
                ...
                {canApprove && r.status === 'pending' && (
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => reviewRequest(r.id, 'approved', r.user_id, r.user_name, r.start_date, r.end_date)}
                      style={{ ...s.btn('#10b981'), padding: '6px 14px', fontSize: 12 }}>Approve</button>
                    <button onClick={() => reviewRequest(r.id, 'denied', r.user_id, r.user_name, r.start_date, r.end_date)}
                      style={{ ...s.btn('#ef4444'), padding: '6px 14px', fontSize: 12 }}>Deny</button>
                  </div>
                )}
              </div>
            </div>
      ...
      {activeView === 'mine' && (
        <div style={s.card}>
          {myRequests.length === 0 && <div style={{ color: '#555', fontSize: 14 }}>You haven't submitted any requests yet.</div>}
          {myRequests.map(r => (
            <div key={r.id} style={{ padding: '14px 0', borderBottom: '1px solid #333' }}>
        ...
        <Modal title="Request Time Off" onClose={() => setShowRequestModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
              ...
              <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} style={{ ...s.input, height: 70, resize: 'vertical' }} placeholder="Any details..." />
            </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
// NOTIFICATIONS BELL (exported for Dashboard use)
// ═══════════════════════════════════════════════════════════════════════════════
  ...
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={() => setOpen(o => !o)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#aaa', fontSize: 20, position: 'relative', padding: '4px 8px' }}>
        🔔
        {unread > 0 && (
          <span style={{ position: 'absolute', top: 0, right: 0, background: '#b22222', color: '#fff', borderRadius: '50%', fontSize: 10, fontWeight: 700, width: 16, height: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {unread > 9 ? '9+' : unread}
          </span>
        )}
      </button>
          ...
          <div style={{ maxHeight: 360, overflowY: 'auto' }}>
            ...
            {notifications.map(n => (
              <div key={n.id} onClick={() => markRead(n.id)}
                style={{ padding: '12px 16px', borderBottom: '1px solid #2a2a2a', background: n.read ? 'transparent' : '#2a1a1a', cursor: 'pointer' }}>
  ...
  return (
    <div style={{ fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', gap: 8, marginBottom: 28, flexWrap: 'wrap' }}>
        ...
        <button style={s.tabBtn(activeTab === 'house')} onClick={() => setActiveTab('house')}>House Events</button>
      ...
      {activeTab === 'house' && <HouseEventsCalendar />}
