...
function WaitingList({ onOpenClient, setActivePage }) {
  ...
  const readyBadge = (date) => {
    ...
    return { label: `${Math.ceil(days / 30)}mo`, color: '#52525b' };
  };
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div style={{ background: '#f7f7f9', borderRadius: 10, border: '1px solid #c9c9cf', overflow: 'hidden' }}>
          {/* Header row */}
          <div style={{ display: 'grid', gridTemplateColumns: '28px 2fr 180px 2fr auto', gap: 8, padding: '10px 14px', background: '#ffffff', borderBottom: '1px solid #b0b0b7' }}>
            ...
            return (
              <div key={client.id} style={{ display: 'grid', gridTemplateColumns: '28px 2fr 180px 2fr auto', gap: 8, padding: '10px 14px', alignItems: 'center', background: idx % 2 === 0 ? 'transparent' : '#f7f7f9', borderBottom: '1px solid #c9c9cf' }}>
                ...
                <div>
                  ...
                  {client.email && <div style={{ fontSize: 12, color: '#71717a' }}>{client.email}</div>}
                </div>
                ...
                <div>
                  {editingId === client.id ? (
                    ...
                    <div onClick={() => hasFullAccess && setEditingId(client.id)}
                      ...
                      <span style={{ fontSize: 13, color: client.ready_date ? '#2563eb' : '#b0b0b7' }}>
                ...
                <div>
                  {editingNoteId === client.id ? (
                    ...
                    <span onClick={() => hasFullAccess && setEditingNoteId(client.id)}
                      style={{ fontSize: 13, color: client.notes ? '#52525b' : '#b0b0b7', cursor: hasFullAccess ? 'pointer' : 'default' }}>
