...
function generateReceiptPDF(payment, client, balance, logoSrc) {
  ...
  const row = (label, value, bold = false) => `
    <div style="display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid #c9c9cf;">
      <span style="font-size:14px;color:#71717a;">${label}</span>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .receipt-title p { font-size: 12px; color: #4b5563; }
...
function generatePaymentHistoryPDF(client, charges, payments, logoSrc) {
  ...
  const timelineRows = timeline.map(t => `
    <tr style="background:${t.type === 'payment' ? '#f0faf4' : '#18181b'};">
      ...
      <td style="padding:9px 12px;border-bottom:1px solid #c9c9cf;text-align:right;font-size:13px;font-weight:600;color:${t.type === 'payment' ? '#16a34a' : '#dc2626'};">
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .report-sub { font-size: 13px; color: #6b7280; margin-bottom: 24px; }
    ...
    .sum-label { font-size: 12px; color: #6b7280; margin-top: 4px; }
  ...
  ${timeline.length === 0 ? '<p style="color:#6b7280;font-size:14px;margin-bottom:20px;">No transactions on record.</p>' : `
  <table>
...
function ClientPayments({ client, onPaymentChange }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
        ...
        {hasFullAccess && (
          <button onClick={() => { setShowAddCharge(!showAddCharge); setShowPaymentForm(false); }}
            style={{ background: showAddCharge ? 'transparent' : 'transparent', border: '1px solid #b8b8bf', color: showAddCharge ? '#52525b' : '#52525b', padding: '8px 16px', borderRadius: '8px', fontSize: '14px', cursor: 'pointer' }}>
      ...
      {/* Record payment form */}
      {showPaymentForm && (
        <div style={{ background: '#f7f7f9', borderRadius: '10px', padding: '16px', marginBottom: '16px', border: '1px solid #c9c9cf' }}>
      ...
      {loading ? <p style={{ color: '#4b5563', fontSize: '14px' }}>Loading...</p> : (
        <>
          ...
          {activeView === 'charges' && (
            charges.length === 0 ? (
              <p style={{ color: '#4b5563', fontSize: '14px' }}>No charges yet.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {charges.map(c => (
                  <div key={c.id} style={{ background: '#ffffff', borderRadius: '8px', padding: '10px 14px', border: '1px solid #c9c9cf', display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                        <span style={{ color: '#18181b', fontSize: '14px', fontWeight: '600' }}>{formatCurrency(c.amount)}</span>
                        <span style={{ fontSize: '12px', padding: '1px 6px', borderRadius: '10px', background: '#f7f7f9', color: '#52525b' }}>
                          {CHARGE_TYPE_LABELS[c.charge_type] || c.charge_type}
                        </span>
                      </div>
                      <div style={{ display: 'flex', gap: '10px' }}>
                        <span style={{ color: '#52525b', fontSize: '13px' }}>{formatDate(c.due_date)}</span>
                        <span style={{ color: '#4b5563', fontSize: '13px' }}>· {c.description}</span>
                      </div>
                    </div>
                    {hasFullAccess && (
          ...
          {activeView === 'payments' && (
            payments.length === 0 ? (
              <p style={{ color: '#4b5563', fontSize: '14px' }}>No payments recorded yet.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {payments.map(p => (
                  <div key={p.id} style={{ background: '#ffffff', borderRadius: '8px', padding: '10px 14px', border: '1px solid #c9c9cf', display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                        ...
                        {p.payer_name && <span style={{ fontSize: '12px', color: '#52525b' }}>{p.payer_name}</span>}
                      </div>
                      <div style={{ display: 'flex', gap: '10px' }}>
                        <span style={{ color: '#52525b', fontSize: '13px' }}>{formatDate(p.payment_date)}</span>
                        {p.notes && <span style={{ color: '#4b5563', fontSize: '13px' }}>· {p.notes}</span>}
                        {p.created_by && <span style={{ color: '#52525b', fontSize: '12px' }}>· by {p.created_by}</span>}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                      <button onClick={() => setShowReceipt(p)}
                        style={{ background: 'transparent', border: '1px solid #b8b8bf', color: '#52525b', padding: '3px 8px', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>
                        Receipt
                      </button>
                      {hasFullAccess && (
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: '20px' }}
          ...
          <div style={{ background: '#18181b', borderRadius: '12px', padding: '32px', maxWidth: '400px', width: '100%', color: '#f4f4f6' }}
            ...
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              ...
              <p style={{ fontSize: '14px', color: '#4b5563', margin: 0 }}>Payment Receipt</p>
            </div>
            <div style={{ borderTop: '1px solid #c9c9cf', borderBottom: '1px solid #c9c9cf', padding: '16px 0', marginBottom: '16px' }}>
              {[
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button onClick={() => setShowReceipt(null)}
                style={{ flex: 1, background: 'transparent', border: '1px solid #52525b', color: '#4b5563', padding: '10px', borderRadius: '8px', fontSize: '14px', cursor: 'pointer' }}>
                Close
              </button>
            </div>
          </div>
