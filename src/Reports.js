...
function StatCard({ label, value, sub, accent }) {
  return (
    <div style={{ background: '#26262e', borderRadius: 12, padding: '16px 18px', borderTop: `3px solid ${accent || '#b22222'}`, minWidth: 0 }}>
...
function Section({ title, children }) {
  return (
    <div style={{ background: '#26262e', borderRadius: 12, padding: '20px 22px', border: '1px solid #32323e', marginBottom: 20 }}>
...
function Row({ label, value }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #32323e' }}>
...
function MetricCard({ label, value, color }) {
  return (
    <div style={{ background: '#1c1c24', borderRadius: 10, padding: '14px 16px' }}>
...
function YearTable({ title, columns, rows }) {
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ background: '#26262e', borderRadius: 12, border: '1px solid #32323e', overflow: 'hidden' }}>
        ...
        {rows.map((row, i) => (
          <div key={row.year} style={{ display: 'grid', gridTemplateColumns: `1fr ${columns.map(() => '90px').join(' ')}`, padding: '11px 16px', gap: 8, borderTop: '1px solid #32323e', background: i === 0 ? '#2a1a1a' : 'transparent' }}>
...
export default function Reports() {
  ...
  const tabBtn = (id) => ({
    padding: '9px 20px', borderRadius: 8, border: '1px solid #3a3a48', cursor: 'pointer', fontSize: 14,
      ...
      {activeTab === 'monthly' && (
        <div>
          <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
            <div>
              ...
              <input type="month" value={reportMonth} onChange={e => setReportMonth(e.target.value)}
                style={{ background: '#26262e', border: '1px solid #3a3a48', borderRadius: 8, padding: '8px 12px', color: '#fff', fontSize: 14 }} />
            </div>
            <div>
          ...
          <Section title="Average Length of Stay (Days)">
            ...
            <div style={{ marginTop: 16, paddingTop: 12, borderTop: '1px solid #32323e', display: 'flex', justifyContent: 'space-between' }}>
...
function PieChart({ data, size = 120 }) {
  ...
  if (total === 0) return <div style={{ width: size, height: size, borderRadius: '50%', background: '#26262e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ color: '#bbb', fontSize: 12 }}>No data</span></div>;
...
function LevelsReport({ clients, houses }) {
  ...
  const ChartCard = ({ title, clientList }) => {
    ...
    return (
      <div style={{ background: '#26262e', borderRadius: 12, padding: '18px 20px', border: '1px solid #32323e' }}>
  ...
  return (
    <div>
      ...
      {/* Pivot table */}
      <div style={{ background: '#26262e', borderRadius: 12, border: '1px solid #32323e', overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #32323e' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: '#252525' }}>
                <th style={{ padding: '10px 16px', textAlign: 'left', color: '#bbb', fontWeight: 600, borderBottom: '1px solid #32323e' }}>House</th>
                <th style={{ padding: '10px 12px', textAlign: 'right', color: '#bbb', fontWeight: 600, borderBottom: '1px solid #32323e' }}>No Level</th>
                {LEVEL_KEYS.map(k => (
                  <th key={k} style={{ padding: '10px 12px', textAlign: 'right', color: LEVEL_COLORS[k], fontWeight: 600, borderBottom: '1px solid #32323e' }}>{k}</th>
                ))}
                <th style={{ padding: '10px 12px', textAlign: 'right', color: '#fff', fontWeight: 700, borderBottom: '1px solid #32323e' }}>Total</th>
              </tr>
            </thead>
            <tbody>
              {pivotRows.map((row, i) => (
...
function MaintenanceReport({ maintenanceRequests }) {
  ...
  const statBox = (label, value, color) => (
    <div style={{ background: '#1e1e26', borderRadius: 10, padding: '18px 24px', border: `1px solid ${color}33` }}>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 28 }}>
        <div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #32323e' }}>
