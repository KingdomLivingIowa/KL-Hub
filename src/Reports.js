...
function StatCard({ label, value, sub, accent }) {
  return (
    <div style={{ background: '#f7f7f9', borderRadius: 12, padding: '16px 18px', borderTop: `3px solid ${accent || '#b22222'}`, minWidth: 0 }}>
      <div style={{ fontSize: 13, color: '#52525b', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</div>
      <div style={{ fontSize: 32, fontWeight: 700, color: '#18181b', lineHeight: 1.1 }}>{value ?? '—'}</div>
      {sub && <div style={{ fontSize: 13, color: '#52525b', marginTop: 5 }}>{sub}</div>}
    </div>
  );
}
...
function Section({ title, children }) {
  return (
    <div style={{ background: '#f7f7f9', borderRadius: 12, padding: '20px 22px', border: '1px solid #c9c9cf', marginBottom: 20 }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: '#52525b', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 16 }}>{title}</div>
      {children}
    </div>
  );
}
...
function Row({ label, value }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #c9c9cf' }}>
...
function MetricCard({ label, value, color }) {
  return (
    <div style={{ background: '#ffffff', borderRadius: 10, padding: '14px 16px' }}>
      <div style={{ fontSize: 13, color: '#52525b', marginBottom: 8 }}>{label}</div>
...
function YearTable({ title, columns, rows }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: '#4b5563', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 10 }}>{title}</div>
      <div style={{ background: '#f7f7f9', borderRadius: 12, border: '1px solid #c9c9cf', overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: `1fr ${columns.map(() => '90px').join(' ')}`, background: '#f7f7f9', padding: '10px 16px', gap: 8 }}>
          <span style={{ fontSize: 12, color: '#52525b', fontWeight: 600 }}>YEAR</span>
          {columns.map(c => <span key={c} style={{ fontSize: 12, color: '#52525b', fontWeight: 600, textAlign: 'right' }}>{c}</span>)}
        </div>
        {rows.map((row, i) => (
          <div key={row.year} style={{ display: 'grid', gridTemplateColumns: `1fr ${columns.map(() => '90px').join(' ')}`, padding: '11px 16px', gap: 8, borderTop: '1px solid #c9c9cf', background: i === 0 ? '#fee2e2' : 'transparent' }}>
            <span style={{ fontSize: 14, color: i === 0 ? '#18181b' : '#52525b', fontWeight: i === 0 ? 600 : 400 }}>{row.year}</span>
            {row.values.map((v, j) => (
              <span key={j} style={{ fontSize: 14, color: i === 0 ? '#18181b' : '#52525b', fontWeight: i === 0 ? 600 : 400, textAlign: 'right' }}>{v ?? '—'}</span>
            ))}
          </div>
        ))}
      </div>
...
function WalkthroughsReport() {
  ...
  const resultBadge = (result) => {
    ...
    const r = map[result] || { label: result, color: '#52525b' };
  ...
  if (loading) return <div style={{ color: '#6b7280', padding: '20px' }}>Loading walkthroughs...</div>;
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <select value={filterHouse} onChange={e => setFilterHouse(e.target.value)}
          style={{ background: '#ffffff', border: '1px solid #b8b8bf', borderRadius: '8px', padding: '7px 12px', color: '#3f3f46', fontSize: '13px' }}>
      ...
      {filtered.length === 0 ? (
        <p style={{ color: '#6b7280', fontSize: '14px' }}>No walkthroughs found.</p>
      ) : (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px', marginBottom: '24px' }}>
            {[
              ...
              <div key={stat.label} style={{ background: '#ffffff', border: '1px solid #c9c9cf', borderRadius: '10px', padding: '14px', position: 'relative', overflow: 'hidden' }}>
                ...
                <p style={{ color: '#6b7280', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.05em', margin: 0 }}>{stat.label}</p>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {filtered.map(w => {
              ...
              return (
                <div key={w.id} style={{ background: '#ffffff', border: '1px solid #c9c9cf', borderRadius: '10px', padding: '12px 16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                    <div>
                      <p style={{ color: '#18181b', fontWeight: '600', fontSize: '14px', margin: '0 0 2px' }}>{w.house_name}</p>
                      <p style={{ color: '#6b7280', fontSize: '12px', margin: 0 }}>{fmtD} · By {w.submitted_by}</p>
                    </div>
...
export default function Reports() {
  ...
  const tabBtn = (id) => ({
    padding: '9px 20px', borderRadius: 8, border: '1px solid #b8b8bf', cursor: 'pointer', fontSize: 14,
    background: activeTab === id ? '#c9c9cf' : 'transparent',
    color: activeTab === id ? '#18181b' : '#52525b',
    fontWeight: activeTab === id ? 600 : 400,
  });

  if (loading) return <div style={{ padding: 32, color: '#52525b', fontSize: 14 }}>Loading reports...</div>;
  ...
  const generateReportPDF = () => {
    ...
    <style>
      *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .stat-label { font-size: 11px; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em; font-weight: 600; }
      ...
      .metric-label { font-size: 11px; color: #4b5563; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 600; line-height: 1.3; }

      /* ── Section blocks ── */
      .section { border: 1px solid #e5e7eb; border-radius: 10px; margin-bottom: 20px; overflow: hidden; }
      .section-title { font-size: 11px; font-weight: 700; color: #4b5563; text-transform: uppercase; letter-spacing: 0.08em; padding: 12px 16px; background: #f9fafb; border-bottom: 1px solid #e5e7eb; }

      /* ── Tables ── */
      table { width: 100%; border-collapse: collapse; }
      th { background: #f3f4f6; text-align: left; padding: 9px 14px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: #4b5563; border-bottom: 1px solid #e5e7eb; }
      ...
      /* ── Year table ── */
      .year-section-title { font-size: 11px; font-weight: 700; color: #4b5563; text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 10px; margin-top: 20px; }
      ...
      .los-overall-label { font-size: 13px; color: #4b5563; }
      ...
      .chart-total { font-size: 11px; color: #6b7280; margin-top: 6px; }
      ...
      .maint-stat-label { font-size: 11px; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em; font-weight: 600; }
      ...
      [style*="color: #52525b"], [style*="color:#52525b"] { color: #4b5563 !important; }
      [style*="color: #52525b"], [style*="color:#52525b"] { color: #4b5563 !important; }
      [style*="color: #4b5563"], [style*="color:#4b5563"] { color: #4b5563 !important; }
      [style*="color: #6b7280"], [style*="color:#6b7280"] { color: #4b5563 !important; }
      [style*="color: #71717a"], [style*="color:#71717a"] { color: #4b5563 !important; }
      ...
      [style*="border: 1px solid #c9c9cf"], [style*="border:1px solid #c9c9cf"],
      [style*="border: 1px solid #c9c9cf"], [style*="border:1px solid #c9c9cf"],
      [style*="border-bottom: 1px solid #c9c9cf"], [style*="border-bottom:1px solid #c9c9cf"],
      [style*="border-bottom: 1px solid #c9c9cf"] { border-color: #e5e7eb !important; }
      ...
      {activeTab === 'monthly' && (
        <div>
          <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
            <div>
              <div style={{ fontSize: 12, color: '#52525b', marginBottom: 5 }}>Report Month</div>
              <input type="month" value={reportMonth} onChange={e => setReportMonth(e.target.value)}
                style={{ background: '#f7f7f9', border: '1px solid #b8b8bf', borderRadius: 8, padding: '8px 12px', color: '#18181b', fontSize: 14 }} />
            </div>
            <div>
              <div style={{ fontSize: 12, color: '#52525b', marginBottom: 5 }}>View</div>
          ...
          <Section title="Average Length of Stay (Days)">
            ...
            <div style={{ marginTop: 16, paddingTop: 12, borderTop: '1px solid #c9c9cf', display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 13, color: '#52525b' }}>Overall average (all time)</span>
...
const LEVEL_COLORS = { L1: '#2563eb', L2: '#b45309', L3: '#db2777', L4: '#059669', null: '#52525b' };
const LEVEL_KEYS = ['L1', 'L2', 'L3', 'L4'];

function PieChart({ data, size = 120 }) {
  const total = data.reduce((s, d) => s + d.count, 0);
  if (total === 0) return <div style={{ width: size, height: size, borderRadius: '50%', background: '#f7f7f9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ color: '#52525b', fontSize: 12 }}>No data</span></div>;
...
function LevelsReport({ clients, houses }) {
  ...
  const ChartCard = ({ title, clientList }) => {
    ...
    return (
      <div style={{ background: '#f7f7f9', borderRadius: 12, padding: '18px 20px', border: '1px solid #c9c9cf' }}>
          ...
          <div>
            {data.filter(d => d.count > 0).map(d => (
              <div key={d.label} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                ...
                <span style={{ fontSize: 12, color: '#52525b' }}>{d.label} ({d.count})</span>
              </div>
            ))}
            <div style={{ fontSize: 11, color: '#52525b', marginTop: 6 }}>{total} residents</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div>
      ...
      {/* Pivot table */}
      <div style={{ background: '#f7f7f9', borderRadius: 12, border: '1px solid #c9c9cf', overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #c9c9cf' }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: '#52525b', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Pivot Table — Levels by House</span>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: '#f7f7f9' }}>
                <th style={{ padding: '10px 16px', textAlign: 'left', color: '#52525b', fontWeight: 600, borderBottom: '1px solid #c9c9cf' }}>House</th>
                <th style={{ padding: '10px 12px', textAlign: 'right', color: '#52525b', fontWeight: 600, borderBottom: '1px solid #c9c9cf' }}>No Level</th>
                {LEVEL_KEYS.map(k => (
                  <th key={k} style={{ padding: '10px 12px', textAlign: 'right', color: LEVEL_COLORS[k], fontWeight: 600, borderBottom: '1px solid #c9c9cf' }}>{k}</th>
                ))}
                <th style={{ padding: '10px 12px', textAlign: 'right', color: '#18181b', fontWeight: 700, borderBottom: '1px solid #c9c9cf' }}>Total</th>
              </tr>
            </thead>
            <tbody>
              {pivotRows.map((row, i) => (
                <tr key={row.name} style={{ background: i % 2 === 0 ? 'transparent' : '#f7f7f9', borderBottom: '1px solid #c9c9cf' }}>
                  <td style={{ padding: '10px 16px', color: '#3f3f46' }}>{row.name}</td>
                  <td style={{ padding: '10px 12px', textAlign: 'right', color: '#52525b' }}>{row.empty}</td>
                  {LEVEL_KEYS.map(k => (
                    <td key={k} style={{ padding: '10px 12px', textAlign: 'right', color: row.counts[k] > 0 ? '#18181b' : '#4b5563' }}>{row.counts[k]}</td>
                  ))}
                  <td style={{ padding: '10px 12px', textAlign: 'right', color: '#18181b', fontWeight: 700 }}>{row.total}</td>
                </tr>
              ))}
              {/* Totals row */}
              <tr style={{ background: '#f7f7f9', borderTop: '2px solid #b0b0b7' }}>
                <td style={{ padding: '10px 16px', color: '#18181b', fontWeight: 700 }}>Total</td>
                <td style={{ padding: '10px 12px', textAlign: 'right', color: '#52525b' }}>{activeClients.filter(c => !c.level).length}</td>
                {LEVEL_KEYS.map(k => (
...
function MaintenanceReport({ maintenanceRequests }) {
  ...
  const statBox = (label, value, color) => (
    <div style={{ background: '#ffffff', borderRadius: 10, padding: '18px 24px', border: `1px solid ${color}33` }}>
      ...
      <div style={{ fontSize: 12, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.07em', marginTop: 4 }}>{label}</div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 28 }}>
        <div>
          <div style={{ fontSize: 13, color: '#52525b', fontWeight: 600, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.07em' }}>By House</div>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #c9c9cf' }}>
                <th style={{ textAlign: 'left', padding: '6px 10px', color: '#71717a', fontWeight: 600 }}>House</th>
            ...
            <tbody>
              {Object.entries(byHouse).sort((a, b) => b[1].total - a[1].total).map(([house, counts]) => (
                <tr key={house} style={{ borderBottom: '1px solid #cfcfd4' }}>
              ...
              {Object.keys(byHouse).length === 0 && (
                <tr><td colSpan="4" style={{ padding: '10px', color: '#71717a', fontStyle: 'italic' }}>No data yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div>
          <div style={{ fontSize: 13, color: '#52525b', fontWeight: 600, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.07em' }}>By Issue Type</div>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #c9c9cf' }}>
                <th style={{ textAlign: 'left', padding: '6px 10px', color: '#71717a', fontWeight: 600 }}>Type</th>
                <th style={{ textAlign: 'right', padding: '6px 8px', color: '#71717a', fontWeight: 600 }}>Count</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(byType).sort((a, b) => b[1] - a[1]).map(([type, count]) => (
                <tr key={type} style={{ borderBottom: '1px solid #cfcfd4' }}>
              ...
              {Object.keys(byType).length === 0 && (
                <tr><td colSpan="2" style={{ padding: '10px', color: '#71717a', fontStyle: 'italic' }}>No data yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div style={{ fontSize: 13, color: '#52525b', fontWeight: 600, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Recent Completions</div>
      {completed.length === 0 ? (
        <p style={{ color: '#71717a', fontSize: 13, fontStyle: 'italic' }}>No completed requests yet.</p>
      ) : (
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #c9c9cf' }}>
              <th style={{ textAlign: 'left', padding: '6px 10px', color: '#71717a', fontWeight: 600 }}>House</th>
              <th style={{ textAlign: 'left', padding: '6px 10px', color: '#71717a', fontWeight: 600 }}>Issue</th>
              <th style={{ textAlign: 'left', padding: '6px 10px', color: '#71717a', fontWeight: 600 }}>Submitted</th>
              <th style={{ textAlign: 'left', padding: '6px 10px', color: '#71717a', fontWeight: 600 }}>Resolved</th>
            </tr>
          </thead>
          <tbody>
            {completed.slice(0, 10).map(r => (
              <tr key={r.id} style={{ borderBottom: '1px solid #cfcfd4' }}>
                <td style={{ padding: '7px 10px', color: '#3f3f46' }}>{r.house_name || '—'}</td>
                <td style={{ padding: '7px 10px', color: '#3f3f46' }}>{r.issue_type || '—'}</td>
                <td style={{ padding: '7px 10px', color: '#6b7280' }}>{r.submitted_at ? new Date(r.submitted_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—'}</td>
